// src/pages/Housekeeping/LostFoundLog.js
 ...
 *  GET    /api/lf-items?q=&status=&room=&from=&to=&propertyCode=&page=&limit=
 *  POST   /api/lf-items
 ...
 *   roomNo,              // "101"
 *   foundLocation,       // "Lobby", "Restaurant"
 *   foundDate,           // ISO datetime
 *   foundById,           // optional user id
 *   foundByName,         // "Ravi"
 *   note,                // optional
 *   status,              // LOGGED | IN_STORAGE | RETURNED | DONATED | DISPOSED
 *   claimantName,        // if returned
 *   claimantContact,     // if returned
 *   returnedDate,        // ISO datetime if returned
 *   propertyCode,        // "TST"
 *   createdAt, updatedAt
 * }
 */
...
const PAGE_SIZE = 10;
const STATUSES = ["LOGGED", "IN_STORAGE", "RETURNED", "DONATED", "DISPOSED"];

export default function LostFoundLog() {
  // list / filters
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      try {
        ...
        const res = await apiFetch(`/api/lf-items?${params.toString()}`, { auth: true });
        ...
        if (!ignore) { setErr(e?.message || "Failed to load lost & found log."); setRows([]); setTotal(0); }
      } finally {
        if (!ignore) setLoading(false);
      }
    })();
    return () => { ignore = true; };
  }, [q, status, room, from, to, propertyCode, page, limit]);
  ...
  const filtered = useMemo(() => {
    ...
    return rows.filter(r =>
      [r.itemName, r.category, r.roomNo, r.foundLocation, r.foundByName, r.note, r.propertyCode]
        .filter(Boolean)
    ...
    <div className="page" style={{ display: "grid", gridTemplateColumns: "auto 1fr" }}>
      ...
      <div className="res-wrap">
        ...
        <div className="res-topbar">
          <h2 style={{ margin: 0 }}>Lost &amp; Found Log</h2>
          <div style={{ display: "flex", gap: 8, flexWrap: "wrap" }}>
            ...
            <select className="res-select" value={status} onChange={(e) => { setStatus(e.target.value); setPage(1); }} style={{ width: 160 }}>
                  ...
                  {dataToRender?.map(r => {
                    ...
                    return (
                      <tr key={id}>
                        ...
                        <td>{fmtDateTime(r.foundDate)}</td>
                        <td>{r.itemName}</td>
                        <td>{r.category || "—"}</td>
                        <td>{r.roomNo || "—"}</td>
                        <td>{r.foundLocation || "—"}</td>
                        <td><StatusBadge value={r.status} /></td>
                        <td title={r.foundById || ""}>{r.foundByName || "—"}</td>
      ...
      {showDelete && (
        <ConfirmModal
          title="Delete Item?"
          message={`Delete "${toDelete?.itemName}" (room ${toDelete?.roomNo || "—"})? This cannot be undone.`}
          confirmText="Delete"
          onClose={() => { setShowDelete(false); setToDelete(null); }}
          onConfirm={async () => {
            const id = toDelete?._id || toDelete?.id;
            await apiFetch(`/api/lf-items/${id}`, { method: "DELETE", auth: true });
            afterDelete(id);
          }}
        />
      )}
...
function LFFormModal({ initial, onClose, onSaved }) {
  ...
  const [foundLocation, setFoundLocation] = useState(initial?.foundLocation || "");
  const [foundDate, setFoundDate] = useState(toDateTimeValue(initial?.foundDate || new Date()));
  const [foundByName, setFoundByName] = useState(initial?.foundByName || "");
  const [foundById, setFoundById] = useState(initial?.foundById || "");
  const [status, setStatus] = useState(initial?.status || "LOGGED");
  const [note, setNote] = useState(initial?.note || "");

  const [claimantName, setClaimantName] = useState(initial?.claimantName || "");
  const [claimantContact, setClaimantContact] = useState(initial?.claimantContact || "");
  const [returnedDate, setReturnedDate] = useState(toDateTimeValue(initial?.returnedDate || ""));
  ...
  const onSubmit = async (e) => {
    ...
    const payload = {
      ...
      foundById: foundById?.trim() || undefined,
      status,
      note: note?.trim() || undefined,
      claimantName: claimantName?.trim() || undefined,
      claimantContact: claimantContact?.trim() || undefined,
      returnedDate: returnedDate ? new Date(returnedDate).toISOString() : undefined,
    ...
    try {
      ...
      if (isEdit) {
        ...
        saved = await apiFetch(`/api/lf-items/${id}`, { method: "PATCH", auth: true, body: JSON.stringify(payload) });
      } else {
        saved = await apiFetch(`/api/lf-items`, { method: "POST", auth: true, body: JSON.stringify(payload) });
      }
      setOk("Saved.");
      onSaved(saved);
    } catch (e2) {
  ...
  return (
    <Modal title={isEdit ? "Edit Lost & Found" : "Add Lost & Found"} onClose={onClose}>
      {err && <Banner type="err">{err}</Banner>}
      ...
      <form onSubmit={onSubmit} style={{ display: "grid", gap: 12 }}>
        <Row>
          ...
          <Field label="Category">
            <input className="input" placeholder="e.g. Personal, Electronics" value={category} onChange={e => setCategory(e.target.value)} />
          </Field>
          <Field label="Status">
        ...
        <Row>
          <Field label="Found Date/Time">
            <input className="input" type="datetime-local" value={foundDate} onChange={e => setFoundDate(e.target.value)} />
          </Field>
          <Field label="Room No">
            <input className="input" value={roomNo} onChange={e => setRoomNo(e.target.value)} />
          </Field>
          <Field label="Found Location">
            <input className="input" value={foundLocation} onChange={e => setFoundLocation(e.target.value)} />
          </Field>
        </Row>

        <Row>
          <Field label="Found By (Name)">
            <input className="input" value={foundByName} onChange={e => setFoundByName(e.target.value)} />
          </Field>
          <Field label="Found By (ID)">
            <input className="input" value={foundById} onChange={e => setFoundById(e.target.value)} />
          </Field>
        ...
        {(status === "RETURNED") && (
          <Row>
            <Field label="Returned Date/Time">
              <input className="input" type="datetime-local" value={returnedDate} onChange={e => setReturnedDate(e.target.value)} />
            </Field>
            <Field label="Claimant Name">
              <input className="input" value={claimantName} onChange={e => setClaimantName(e.target.value)} />
            </Field>
            <Field label="Claimant Contact">
              <input className="input" value={claimantContact} onChange={e => setClaimantContact(e.target.value)} />
            </Field>
          </Row>
        )}
...
function StatusBadge({ value }) {
  ...
  const palette = {
    LOGGED:     { bg: "#eff6ff", bd: "#bfdbfe", fg: "#1d4ed8" },
    IN_STORAGE: { bg: "#fefce8", bd: "#fde68a", fg: "#92400e" },
    RETURNED:   { bg: "#f0fdf4", bd: "#bbf7d0", fg: "#166534" },
    DONATED:    { bg: "#ecfeff", bd: "#a5f3fc", fg: "#155e75" },
    DISPOSED:   { bg: "#fef2f2", bd: "#fecaca", fg: "#b91c1c" },
