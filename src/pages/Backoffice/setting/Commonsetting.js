// src/pages/Backoffice/setting/CommonSetting.js
...
export default function CommonSetting() {
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      try {
        ...
        const res = await apiFetch(`/api/common-settings?${params.toString()}`, { auth: true });
        ...
        if (!ignore) {
          setErr(e?.message || "Failed to load common settings.");
          setRows([]); setTotal(0);
        }
      } finally {
  ...
  const filtered = useMemo(() => {
    ...
    return rows.filter(r =>
      [
        r.propertyCode, r.currency, r.currencySymbol, r.language, r.timezone,
        r.dateFormat, r.timeFormat,
      ]
        .filter(Boolean)
    ...
    <div className="page" style={{ display: "grid", gridTemplateColumns: "auto 1fr" }}>
      ...
      <div className="res-wrap">
        ...
        <div className="res-topbar">
          <h2 style={{ margin: 0 }}>Common Settings</h2>
          <div style={{ display: "flex", gap: 8 }}>
            <input
              className="res-select"
              placeholder="Search (property / format / currency / language / tz)"
              value={q}
                  ...
                  {dataToRender?.map(r => {
                    ...
                    return (
                      <tr key={id}>
                        ...
                        <td>{r.dateFormat} / {r.timeFormat}</td>
                        <td>{r.currency} {r.currencySymbol ? `(${r.currencySymbol})` : ""}</td>
                        <td>{r.language}</td>
                        <td>{r.timezone}</td>
                        <td><OnOff value={r.enableGST} /></td>
                        <td>{r.enableServiceCharge ? `${r.serviceChargePct ?? 0}%` : "—"}</td>
      ...
      {showDelete && (
        <ConfirmModal
          ...
          onConfirm={async () => {
            ...
            await apiFetch(`/api/common-settings/${id}`, { method: "DELETE", auth: true });
            afterDelete(id);
          }}
        />
      )}
...
/* ---------- Form Modal ---------- */
function CommonSettingForm({ initial, onClose, onSaved }) {
  ...
  const [autoLogoutMinutes, setAutoLogoutMinutes] = useState(initial?.autoLogoutMinutes ?? 0);

  const [enableGST, setEnableGST] = useState(initial?.enableGST ?? true);
  const [enableServiceCharge, setEnableServiceCharge] = useState(initial?.enableServiceCharge ?? false);
  const [serviceChargePct, setServiceChargePct] = useState(initial?.serviceChargePct ?? 0);
  ...
  const onSubmit = async (e) => {
    ...
    const payload = {
      ...
      isActive,
      dateFormat, timeFormat, currency, currencySymbol, language, timezone,
      decimalPlaces: Number(decimalPlaces || 0),
      autoLogoutMinutes: Number(autoLogoutMinutes || 0),
      enableGST: !!enableGST,
    ...
    try {
      ...
      if (isEdit) {
        ...
        saved = await apiFetch(`/api/common-settings/${id}`, {
          method: "PATCH", auth: true, body: JSON.stringify(payload),
        });
      } else {
        // Upsert in the backend ensures one-per-property
        saved = await apiFetch("/api/common-settings", {
  ...
  return (
    <Modal title={isEdit ? "Edit Common Setting" : "Create Common Setting"} onClose={onClose}>
      {err && <Banner type="err">{err}</Banner>}
      ...
      <form onSubmit={onSubmit} style={{ display: "grid", gap: 12 }}>
        ...
        <Row>
          ...
          <Field label="Time Format">
            <select className="res-select" value={timeFormat} onChange={(e) => setTimeFormat(e.target.value)}>
        ...
        <Row>
          <Field label="Decimal Places">
            <input className="input" type="number" min="0" max="4" value={decimalPlaces}
                   onChange={(e) => setDecimalPlaces(e.target.value)} />
          </Field>
          <Field label="Auto Logout (minutes)">
            <input className="input" type="number" min="0" value={autoLogoutMinutes}
                   onChange={(e) => setAutoLogoutMinutes(e.target.value)} />
          </Field>
        </Row>

        <div className="panel" style={{ marginTop: 8 }}>
          <div className="panel-h">Charges</div>
            ...
            <Row>
              <Field label="Enable GST">
                <label style={{ display: "flex", alignItems: "center", gap: 8 }}>
                  ...
                  <span>{enableGST ? "Yes" : "No"}</span>
                </label>
              </Field>
              <Field label="Enable Service Charge">
                <label style={{ display: "flex", alignItems: "center", gap: 8 }}>
                  <input type="checkbox" checked={enableServiceCharge} onChange={e => setEnableServiceCharge(e.target.checked)} />
                  <span>{enableServiceCharge ? "Yes" : "No"}</span>
                </label>
              </Field>
