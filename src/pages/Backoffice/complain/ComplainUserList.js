...
export default function ComplainUserList() {
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      try {
        ...
        const res = await apiFetch(`/api/complain-users?${params.toString()}`, { auth: true });
        ...
        if (!ignore) { setErr(e?.message || "Failed to load complain users."); setRows([]); setTotal(0); }
      } finally {
  ...
  const filtered = useMemo(() => {
    ...
    return rows.filter((r) =>
      [r.code, r.name, r.email, r.phone, r.role]
        .filter(Boolean)
    ...
    <div className="page" style={{ display: "grid", gridTemplateColumns: "auto 1fr" }}>
      ...
      <div className="res-wrap">
        ...
        <div className="res-topbar">
          <div>
            <h2 style={{ margin: 0 }}>Complain User List</h2>
          ...
          <div style={{ display: "flex", gap: 8 }}>
            <input
              ...
              placeholder="Search (code / name / email / phone / role)"
              value={q}
              ...
              <table className="table">
                ...
                <tbody>
                  {(!dataToRender || dataToRender.length === 0) && !loading && (
                    <tr className="no-rows"><td colSpan={9}>No users found</td></tr>
                  )}
      ...
      {showDelete && (
        <ConfirmModal
          title="Delete Complain User?"
          message={`Delete "${toDelete?.name}" (${toDelete?.code})? This cannot be undone.`}
          confirmText="Delete"
          onClose={() => { setShowDelete(false); setToDelete(null); }}
          onConfirm={async () => {
            const id = toDelete?._id || toDelete?.id;
            await apiFetch(`/api/complain-users/${id}`, { method: "DELETE", auth: true });
            afterDelete(id);
          }}
        />
      )}
...
/* ---------- Form Modal ---------- */
function ComplainUserFormModal({ initial, onClose, onSaved, propertyCode }) {
  ...
  const onSubmit = async (e) => {
    ...
    try {
      ...
      if (isEdit) {
        ...
        saved = await apiFetch(`/api/complain-users/${id}`, {
          method: "PATCH", auth: true, body: JSON.stringify(payload),
        });
      } else {
        saved = await apiFetch("/api/complain-users", {
  ...
  return (
    <Modal title={isEdit ? "Edit Complain User" : "Add Complain User"} onClose={onClose}>
      {err && <Banner type="err">{err}</Banner>}
      ...
      <form onSubmit={onSubmit} style={{ display: "grid", gap: 12 }}>
        ...
        <Row>
          ...
          <Field label="Role">
            <input className="input" value={role} onChange={(e) => setRole(e.target.value)} placeholder="e.g., Agent / Manager" />
          </Field>
        </Row>
