...
export default function ComplainType() {
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      try {
        ...
        const res = await apiFetch(`/api/complain-types?${params.toString()}`, { auth: true });
        ...
        if (!ignore) { setErr(e?.message || "Failed to load complain types."); setRows([]); setTotal(0); }
      } finally {
    ...
    <div className="page" style={{ display: "grid", gridTemplateColumns: "auto 1fr" }}>
      ...
      <div className="res-wrap">
        ...
        <div className="res-topbar">
          <div>
            <h2 style={{ margin: 0 }}>Complain Type</h2>
              ...
              <table className="table">
                ...
                <tbody>
                  {(!dataToRender || dataToRender.length === 0) && !loading && (
                    <tr className="no-rows"><td colSpan={7}>No complain types found</td></tr>
                  )}
      ...
      {showDelete && (
        <ConfirmModal
          title="Delete Complain Type?"
          message={`Delete "${toDelete?.name}" (${toDelete?.code})? This cannot be undone.`}
          confirmText="Delete"
          onClose={() => { setShowDelete(false); setToDelete(null); }}
          onConfirm={async () => {
            const id = toDelete?._id || toDelete?.id;
            await apiFetch(`/api/complain-types/${id}`, { method: "DELETE", auth: true });
            afterDelete(id);
          }}
        />
      )}
...
/* ---------- Form Modal ---------- */
function ComplainTypeFormModal({ initial, onClose, onSaved, propertyCode }) {
  ...
  const onSubmit = async (e) => {
    ...
    try {
      ...
      if (isEdit) {
        ...
        saved = await apiFetch(`/api/complain-types/${id}`, {
          method: "PATCH", auth: true, body: JSON.stringify(payload),
        });
      } else {
        saved = await apiFetch("/api/complain-types", {
      ...
      setErr(e2?.message || "Failed to save complain type.");
    } finally {
      setSaving(false);
    }
  };
  ...
  return (
    <Modal title={isEdit ? "Edit Complain Type" : "Create Complain Type"} onClose={onClose}>
      {err && <Banner type="err">{err}</Banner>}
