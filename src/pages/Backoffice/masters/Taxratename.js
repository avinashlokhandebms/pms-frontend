                  ...
                  {dataToRender?.map(r => {
                    ...
                    return (
                      <tr key={id}>
                        ...
                        <td><OnOff value={r.isInclusive} /></td>
                        <td>{r.displayOrder ?? 0}</td>
...
function TaxRateNameForm({ initial, onClose, onSaved }) {
  ...
  return (
    <Modal title={isEdit ? "Edit Tax Rate Name" : "Create Tax Rate Name"} onClose={onClose}>
      {err && <Banner type="err">{err}</Banner>}
        ...
        <Row>
          <Field label="Inclusive">
            <label style={{ display: "flex", alignItems: "center", gap: 8 }}>
              ...
              <span>{isInclusive ? "Yes" : "No"}</span>
            </label>
          </Field>
          <Field label="Display Order">
            <input className="input" type="number" value={displayOrder} onChange={e => setDisplayOrder(e.target.value)} />
          </Field>
        </Row>
...
/* ---------- Tiny UI helpers (same as your other masters) ---------- */
