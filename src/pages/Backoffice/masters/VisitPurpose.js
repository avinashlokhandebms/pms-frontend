    ...
    <div className="page" style={{ display: "grid", gridTemplateColumns: "auto 1fr" }}>
              ...
              <table className="table">
                ...
                <tbody>
                  {(!dataToRender || dataToRender.length === 0) && !loading && (
                    <tr className="no-rows"><td colSpan={7}>No visit purposes found</td></tr>
                  )}
      ...
      {showDelete && (
        <ConfirmModal
          ...
          message={`Delete purpose "${toDelete?.name}" (${toDelete?.code})? This cannot be undone.`}
          confirmText="Delete"
...
/* ---------- Form Modal ---------- */
function PurposeFormModal({ initial, onClose, onSaved }) {
  ...
  const onSubmit = async (e) => {
    ...
    try {
      ...
      if (isEdit) {
        ...
        saved = await apiFetch(`/api/visitpurposes/${id}`, { method: "PATCH", auth: true, body: JSON.stringify(payload) });
      } else {
        saved = await apiFetch("/api/visitpurposes", { method: "POST", auth: true, body: JSON.stringify(payload) });
      }
      setOk("Saved.");
      onSaved(saved);
    } catch (e2) {
      setErr(e2?.message || "Failed to save purpose.");
    } finally {
      setSaving(false);
    }
  };
  ...
  return (
    <Modal title={isEdit ? "Edit Purpose" : "Create Purpose"} onClose={onClose}>
      {err && <Banner type="err">{err}</Banner>}
