// src/pages/Backoffice/common-master/VegType.js
...
export default function VegTypePage() {
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      try {
        ...
        const res = await apiFetch(`/api/veg-types?${params.toString()}`, { auth: true });
        const data = res?.data || res?.items || res || [];
        const count = res?.total ?? data.length ?? 0;
        if (!ignore) { setRows(Array.isArray(data) ? data : []); setTotal(Number(count) || 0); }
      } catch (e) {
        if (!ignore) { setErr(e?.message || "Failed to load veg types."); setRows([]); setTotal(0); }
      } finally { if (!ignore) setLoading(false); }
    })();
    return () => { ignore = true; };
  }, [q, page, limit]);

  // Client fallback filter
              ...
              <table className="table">
                ...
                <tbody>
                  {(!dataToRender || dataToRender.length === 0) && !loading && (
                    <tr className="no-rows"><td colSpan={9}>No veg types found</td></tr>
                  )}
                      ...
                      <tr key={id}>
                        ...
                        <td>
                          <span style={{
                            ...
                            border:"1px solid #e5e7eb", borderRadius:8, padding:"2px 8px"
                          }}>
                            <span style={{
                              width:14, height:14, borderRadius:4,
                              background: r.color || "#16a34a", border:"1px solid #e5e7eb"
                            }} />
                            <span className="small">{r.color || "#16a34a"}</span>
                          </span>
                        </td>
                        <td>{r.sortOrder ?? 0}</td>
...
/* ---------- Form Modal ---------- */
function VegTypeFormModal({ initial, onClose, onSaved }) {
  ...
  const [color, setColor] = useState(initial?.color || "#16a34a"); // default green
  ...
  const onSubmit = async (e) => {
    ...
    try {
      ...
      if (isEdit) {
        ...
        saved = await apiFetch(`/api/veg-types/${id}`, {
          method: "PATCH", auth: true, body: JSON.stringify(payload),
        });
      } else {
        saved = await apiFetch("/api/veg-types", {
      ...
      setErr(e2?.message || "Failed to save veg type.");
    } finally { setSaving(false); }
  };

  return (
    <Modal title={isEdit ? "Edit Veg Type" : "Add Veg Type"} onClose={onClose}>
      {err && <Banner type="err">{err}</Banner>}
      ...
      <form onSubmit={onSubmit} style={{ display: "grid", gap: 12 }}>
        <Row>
          ...
          <Field label="Code" required>
            <input className="input" value={code} onChange={e => setCode(e.target.value)} placeholder="e.g. VEG / NVG / EGG" />
          </Field>
          <Field label="Name" required>
            <input className="input" value={name} onChange={e => setName(e.target.value)} placeholder="e.g. Vegetarian" />
          </Field>
        </Row>
        ...
        <Row>
          <Field label="Color">
            <div style={{ display: "flex", gap: 8, alignItems: "center" }}>
              <input type="color" value={color} onChange={e => setColor(e.target.value)} style={{ width: 42, height: 32, padding: 0, border: "1px solid #e5e7eb", borderRadius: 6 }} />
              <input className="input" value={color} onChange={e => setColor(e.target.value)} placeholder="#16a34a" />
            </div>
          </Field>
        ...
        <Row>
          <Field label="Description">
            <textarea className="input" rows={2} value={description} onChange={e => setDescription(e.target.value)} placeholder="(optional) e.g. Pure veg only" />
          </Field>
        </Row>
