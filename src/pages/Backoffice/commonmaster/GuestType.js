// src/pages/Backoffice/common-master/GuestType.js
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      try {
        ...
        const res = await apiFetch(`/api/guest-types?${params.toString()}`, { auth: true });
        ...
        if (!ignore) { setErr(e?.message || "Failed to load guest types."); setRows([]); setTotal(0); }
    ...
    <div className="page" style={{ display: "grid", gridTemplateColumns: "auto 1fr" }}>
      ...
      <div className="res-wrap">
        <div className="res-topbar">
          <h2 style={{ margin: 0 }}>Guest Type</h2>
          <div style={{ display: "flex", gap: 8 }}>
            ...
            <button className="btn" onClick={openCreate}>+ Add Guest Type</button>
          </div>
        </div>
              ...
              <table className="table">
                ...
                <tbody>
                  {(!dataToRender || dataToRender.length === 0) && !loading && (
                    <tr className="no-rows"><td colSpan={8}>No guest types found</td></tr>
                  )}
      ...
      {showDelete && (
        <ConfirmModal
          ...
          onConfirm={async () => {
            ...
            await apiFetch(`/api/guest-types/${id}`, { method: "DELETE", auth: true });
            afterDelete(id);
          }}
        />
      )}
...
/* ---------- Form Modal ---------- */
function GuestTypeForm({ initial, onClose, onSaved }) {
  ...
  const onSubmit = async (e) => {
    ...
    try {
      ...
      if (isEdit) {
        ...
        saved = await apiFetch(`/api/guest-types/${id}`, { method: "PATCH", auth: true, body: JSON.stringify(payload) });
      } else {
        saved = await apiFetch("/api/guest-types", { method: "POST", auth: true, body: JSON.stringify(payload) });
      }
      setOk("Saved.");
      onSaved(saved);
    } catch (e2) {
      setErr(e2?.message || "Failed to save guest type.");
    } finally {
      setSaving(false);
    }
  };
  ...
  return (
    <Modal title={isEdit ? "Edit Guest Type" : "Create Guest Type"} onClose={onClose}>
      {err && <Banner type="err">{err}</Banner>}
      ...
      <form onSubmit={onSubmit} style={{ display: "grid", gap: 12 }}>
        <Row>
          ...
          <Field label="Code" required>
            <input className="input" value={code} onChange={(e) => setCode(e.target.value)} placeholder="e.g. INDIV, CORP, GOVT" />
          </Field>
          <Field label="Name" required>
            <input className="input" value={name} onChange={(e) => setName(e.target.value)} placeholder="Individual / Corporate / Government" />
          </Field>
        </Row>
