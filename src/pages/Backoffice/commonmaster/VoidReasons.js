// src/pages/Backoffice/common-master/VoidReasons.js
...
export default function VoidReasonsPage() {
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      try {
        ...
        const res = await apiFetch(`/api/void-reasons?${params.toString()}`, { auth: true });
        const data = res?.data || res?.items || res || [];
        const count = res?.total ?? data.length ?? 0;
        if (!ignore) { setRows(Array.isArray(data) ? data : []); setTotal(Number(count) || 0); }
      } catch (e) {
        if (!ignore) { setErr(e?.message || "Failed to load void reasons."); setRows([]); setTotal(0); }
  ...
  const filtered = useMemo(() => {
    ...
    return rows.filter(r =>
      [r.propertyCode, r.code, r.name, r.category, r.description]
    ...
    <div className="page" style={{ display: "grid", gridTemplateColumns: "auto 1fr" }}>
      ...
      <div className="res-wrap">
        ...
        <div className="res-topbar">
          <h2 style={{ margin: 0 }}>Void Reasons</h2>
          <div style={{ display: "flex", gap: 8 }}>
            <input
              className="res-select"
              placeholder="Search (property / code / name / category)"
              value={q}
            ...
            <button className="btn" onClick={openCreate}>+ Add Reason</button>
          </div>
        </div>
              ...
              <table className="table">
                ...
                <tbody>
                  {(!dataToRender || dataToRender.length === 0) && !loading && (
                    <tr className="no-rows"><td colSpan={7}>No reasons found</td></tr>
                  )}
      ...
      {showDelete && (
        <ConfirmModal
          ...
          onConfirm={async () => {
            ...
            await apiFetch(`/api/void-reasons/${id}`, { method: "DELETE", auth: true });
            afterDelete(id);
          }}
        />
      )}
...
/* ---------- Form ---------- */
function VoidReasonForm({ initial, onClose, onSaved }) {
  ...
  const [category, setCategory] = useState(initial?.category || "GENERAL"); // GENERAL | POS | FNB | ROOM
  ...
  const onSubmit = async (e) => {
    ...
    try {
      ...
      if (isEdit) {
        ...
        saved = await apiFetch(`/api/void-reasons/${id}`, {
          method: "PATCH", auth: true, body: JSON.stringify(payload),
        });
      } else {
        saved = await apiFetch("/api/void-reasons", {
  ...
  return (
    <Modal title={isEdit ? "Edit Void Reason" : "Add Void Reason"} onClose={onClose}>
      {err && <Banner type="err">{err}</Banner>}
        ...
        <Row>
          <Field label="Category">
            <select className="res-select" value={category} onChange={(e) => setCategory(e.target.value)}>
              {["GENERAL", "POS", "FNB", "ROOM"].map(k => <option key={k} value={k}>{k}</option>)}
            </select>
          </Field>
          <Field label="Active">
