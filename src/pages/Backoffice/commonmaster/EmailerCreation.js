// src/pages/Backoffice/common-master/EmailerCreation.js
...
export default function EmailerCreationPage() {
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      try {
        ...
        const res = await apiFetch(`/api/emailers?${params.toString()}`, { auth: true });
        const data = res?.data || res?.items || res || [];
        const count = res?.total ?? data.length ?? 0;
        if (!ignore) { setRows(Array.isArray(data) ? data : []); setTotal(Number(count) || 0); }
      } catch (e) {
        if (!ignore) { setErr(e?.message || "Failed to load emailers."); setRows([]); setTotal(0); }
  ...
  const filtered = useMemo(() => {
    ...
    return rows.filter(r =>
      [r.propertyCode, r.code, r.name, r.subject, r.category]
        .filter(Boolean)
    ...
    <div className="page" style={{ display: "grid", gridTemplateColumns: "auto 1fr" }}>
      ...
      <div className="res-wrap">
        ...
        <div className="res-topbar">
          <h2 style={{ margin: 0 }}>Emailer Creation</h2>
          <div style={{ display: "flex", gap: 8 }}>
            <input
              className="res-select"
              placeholder="Search (property / code / name / subject / category)"
              value={q}
              ...
              <table className="table">
                ...
                <tbody>
                  {(!dataToRender || dataToRender.length === 0) && !loading && (
                    <tr className="no-rows"><td colSpan={9}>No emailers found</td></tr>
                  )}
                  ...
                  {dataToRender?.map(r => {
                    ...
                    return (
                      <tr key={id}>
                        ...
                        <td>{r.name}</td>
                        <td>{r.category || "—"}</td>
                        <td title={r.subject || ""} style={{ maxWidth: 260, overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>
                          {r.subject || "—"}
                        </td>
...
/* ---------- Form Modal ---------- */
function EmailerFormModal({ initial, onClose, onSaved }) {
  ...
  const onSubmit = async (e) => {
    ...
    const payload = {
      ...
      cc: csvToArr(cc),
      bcc: csvToArr(bcc),
      variables: csvToArr(variables), // e.g., FIRST_NAME, BOOKING_ID
      bodyHtml,
      isActive: !!isActive,
    };
    ...
    try {
      ...
      if (isEdit) {
        ...
        saved = await apiFetch(`/api/emailers/${id}`, {
          method: "PATCH", auth: true, body: JSON.stringify(payload),
        });
      } else {
        saved = await apiFetch("/api/emailers", {
      ...
      setErr(e2?.message || "Failed to save emailer.");
    } finally { setSaving(false); }
  };

  return (
    <Modal title={isEdit ? "Edit Emailer" : "Add Emailer"} onClose={onClose}>
      {err && <Banner type="err">{err}</Banner>}
      ...
      <form onSubmit={onSubmit} style={{ display: "grid", gap: 12 }}>
        <Row>
          ...
          <Field label="Code" required>
            <input className="input" value={code} onChange={e => setCode(e.target.value)} placeholder="e.g. RESV_CONF" />
          </Field>
          <Field label="Name" required>
            <input className="input" value={name} onChange={e => setName(e.target.value)} placeholder="e.g. Reservation Confirmation" />
          </Field>
        </Row>
        ...
        <Row>
          <Field label="Category">
            <select className="res-select" value={category} onChange={e => setCategory(e.target.value)}>
              {["GENERAL", "RESERVATION", "INVOICE", "PAYMENT", "USER", "MARKETING"].map(c => (
        ...
        <Row>
          <Field label="From Name">
            <input className="input" value={fromName} onChange={e => setFromName(e.target.value)} placeholder="Hotel Trustify" />
          </Field>
          <Field label="From Email" required>
            <input className="input" value={fromEmail} onChange={e => setFromEmail(e.target.value)} placeholder="noreply@hotel.com" />
          </Field>
          <Field label="Reply-To">
            <input className="input" value={replyTo} onChange={e => setReplyTo(e.target.value)} placeholder="support@hotel.com" />
          </Field>
        </Row>

        <Row>
          <Field label="CC (comma separated)">
            <input className="input" value={cc} onChange={e => setCc(e.target.value)} placeholder="cc1@x.com, cc2@x.com" />
          </Field>
          <Field label="BCC (comma separated)">
            <input className="input" value={bcc} onChange={e => setBcc(e.target.value)} placeholder="bcc1@x.com" />
          </Field>
          <Field label="Variables (comma separated)">
            <input className="input" value={variables} onChange={e => setVariables(e.target.value)} placeholder="FIRST_NAME, BOOKING_ID, CHECKIN_DATE" />
          </Field>
        </Row>
          ...
          <div className="panel-b" style={{ display: "grid", gap: 10 }}>
            {!showPreview ? (
              <textarea
                ...
                placeholder={`Use variables like {{FIRST_NAME}}.\nExample: <h1>Welcome {{FIRST_NAME}}</h1>`}
              />
            ) : (
              <div
                style={{ border: "1px solid #e5e7eb", borderRadius: 10, padding: 12, minHeight: 160, background: "#fff" }}
                dangerouslySetInnerHTML={{ __html: safePreview(bodyHtml) }}
              />
            )}
          </div>
        </div>
...
function safePreview(html) {
  // very light guard for preview (not a sanitizer) – server should sanitize/output safely when sending
  return html || "<em>No content</em>";
}
