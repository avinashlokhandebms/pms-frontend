// src/pages/Backoffice/common-master/Unit.js
...
export default function UnitPage() {
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      try {
        ...
        if (!ignore) { setErr(e?.message || "Failed to load units."); setRows([]); setTotal(0); }
  ...
  const filtered = useMemo(() => {
    ...
    return rows.filter(r =>
      [r.code, r.name, r.symbol, r.description, r.propertyCode]
        .filter(Boolean)
    ...
    <div className="page" style={{ display: "grid", gridTemplateColumns: "auto 1fr" }}>
      ...
      <div className="res-wrap">
        <div className="res-topbar">
          <h2 style={{ margin: 0 }}>Unit</h2>
          <div style={{ display: "flex", gap: 8 }}>
            <input
              className="res-select"
              placeholder="Search (code / name / symbol / property)"
              value={q}
            ...
            <button className="btn" onClick={openCreate}>+ Add Unit</button>
          </div>
        </div>
              ...
              <table className="table">
                ...
                <tbody>
                  {(!dataToRender || dataToRender.length === 0) && !loading && (
                    <tr className="no-rows"><td colSpan={10}>No units found</td></tr>
                  )}
                  {dataToRender?.map(r => {
                    const id = r._id || r.id;
                    return (
      ...
      {showDelete && (
        <ConfirmModal
          title="Delete Unit?"
          message={`Delete unit "${toDelete?.name}" (${toDelete?.code})? This cannot be undone.`}
          confirmText="Delete"
          onClose={() => { setShowDelete(false); setToDelete(null); }}
          onConfirm={async () => {
            const id = toDelete?._id || toDelete?.id;
            await apiFetch(`/api/units/${id}`, { method: "DELETE", auth: true });
            afterDelete(id);
          }}
        />
      )}
...
/* ---------- Form Modal ---------- */
function UnitFormModal({ initial, onClose, onSaved }) {
  ...
  const onSubmit = async (e) => {
    ...
    if (Number(decimalPlaces) < 0 || Number(decimalPlaces) > 6) return setErr("Decimals must be 0–6");
    ...
    try {
      ...
      if (isEdit) {
        ...
        saved = await apiFetch(`/api/units/${id}`, { method: "PATCH", auth: true, body: JSON.stringify(payload) });
      } else {
        saved = await apiFetch("/api/units", { method: "POST", auth: true, body: JSON.stringify(payload) });
      }
      setOk("Saved.");
      onSaved(saved);
    } catch (e2) {
      setErr(e2?.message || "Failed to save unit.");
    } finally { setSaving(false); }
  };

  return (
    <Modal title={isEdit ? "Edit Unit" : "Create Unit"} onClose={onClose}>
      {err && <Banner type="err">{err}</Banner>}
      ...
      <form onSubmit={onSubmit} style={{ display: "grid", gap: 12 }}>
        <Row>
          ...
          <Field label="Code" required>
            <input className="input" value={code} onChange={(e) => setCode(e.target.value)} placeholder="KG" />
          </Field>
