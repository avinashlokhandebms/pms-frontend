// src/pages/Backoffice/common-master/Brand.js
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      try {
        ...
        const res = await apiFetch(`/api/brands?${params.toString()}`, { auth: true });
        const data = res?.data || res?.items || res || [];
        const count = res?.total ?? data.length ?? 0;
        if (!ignore) { setRows(Array.isArray(data) ? data : []); setTotal(Number(count) || 0); }
      } catch (e) {
        if (!ignore) { setErr(e?.message || "Failed to load brands."); setRows([]); setTotal(0); }
      ...
      <div className="res-wrap">
        ...
        <div className="res-topbar">
          ...
          <div style={{ display: "flex", gap: 8 }}>
            ...
            <button className="btn" onClick={openCreate}>+ Add Brand</button>
          </div>
        </div>
              ...
              <table className="table">
                ...
                <tbody>
                  {(!dataToRender || dataToRender.length === 0) && !loading && (
                    <tr className="no-rows"><td colSpan={8}>No brands found</td></tr>
                  )}
      ...
      {showDelete && (
        <ConfirmModal
          ...
          onConfirm={async () => {
            ...
            await apiFetch(`/api/brands/${id}`, { method: "DELETE", auth: true });
            afterDelete(id);
          }}
        />
      )}
...
function BrandForm({ initial, onClose, onSaved }) {
  ...
  const onSubmit = async (e) => {
    ...
    try {
      ...
      if (isEdit) {
        ...
        saved = await apiFetch(`/api/brands/${id}`, { method: "PATCH", auth: true, body: JSON.stringify(payload) });
      } else {
        saved = await apiFetch("/api/brands", { method: "POST", auth: true, body: JSON.stringify(payload) });
      }
      setOk("Saved.");
      onSaved(saved);
    } catch (e2) {
      setErr(e2?.message || "Failed to save brand.");
    } finally {
      setSaving(false);
    }
  };
  ...
  return (
    <Modal title={isEdit ? "Edit Brand" : "Create Brand"} onClose={onClose}>
      {err && <Banner type="err">{err}</Banner>}
      ...
      <form onSubmit={onSubmit} style={{ display: "grid", gap: 12 }}>
        <Row>
          ...
          <Field label="Code" required>
            <input className="input" value={code} onChange={(e) => setCode(e.target.value)} placeholder="e.g. BRND01" />
          </Field>
          <Field label="Name" required>
            <input className="input" value={name} onChange={(e) => setName(e.target.value)} placeholder="Brand name" />
          </Field>
        </Row>
