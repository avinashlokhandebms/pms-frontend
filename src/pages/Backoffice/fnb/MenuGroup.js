// src/pages/Backoffice/fnb/MenuGroup.js
    ...
    (async () => {
      try {
        ...
        if (!ignore) {
          ...
          const map = {};
          list.forEach(c => { map[c._id || c.id] = c; });
          setCatMap(map);
        }
      } catch {
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      try {
        ...
        if (!ignore) { setErr(e?.message || "Failed to load menu groups."); setRows([]); setTotal(0); }
      } finally {
    ...
    <div className="page" style={{ display: "grid", gridTemplateColumns: "auto 1fr" }}>
      ...
      <div className="res-wrap">
        ...
        <div className="res-topbar">
          <h2 style={{ margin: 0 }}>FNB — Menu Groups</h2>
              ...
              <table className="table">
                ...
                <tbody>
                  {(!dataToRender || dataToRender.length === 0) && !loading && (
                    <tr className="no-rows"><td colSpan={8}>No menu groups found</td></tr>
                  )}

                  {dataToRender?.map(r => {
                    const id = r._id || r.id;
                    const cId = r.categoryId || r.category?._id || r.category?.id;
                    const category = catMap[cId] || r.category;
                    const categoryName = category?.name || r.categoryName || r.categoryCode || "—";
                    return (
                      <tr key={id}>
                        <td>
      ...
      {showDelete && (
        <ConfirmModal
          ...
          onConfirm={async () => {
            ...
            await apiFetch(`/api/fnb/menu-groups/${id}`, { method: "DELETE", auth: true });
            afterDelete(id);
          }}
        />
      )}
...
/* ---------- Form Modal ---------- */
function MenuGroupFormModal({ initial, defaultPropertyCode, categories, onClose, onSaved }) {
  ...
  const onSubmit = async (e) => {
    ...
    try {
      ...
      if (isEdit) {
        ...
        saved = await apiFetch(`/api/fnb/menu-groups/${id}`, {
          method: "PATCH", auth: true, body: JSON.stringify(payload),
        });
      } else {
        saved = await apiFetch("/api/fnb/menu-groups", {
      ...
      setErr(e2?.message || "Failed to save menu group.");
    } finally {
      setSaving(false);
    }
  };
  ...
  return (
    <Modal title={isEdit ? "Edit Menu Group" : "Create Menu Group"} onClose={onClose}>
      {err && <Banner type="err">{err}</Banner>}
