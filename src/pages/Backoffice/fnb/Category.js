// src/pages/Backoffice/fnb/Category.js
...
export default function CategoryPage() {
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      try {
        const res = await apiFetch(`/api/fnb/parent-categories?${params.toString()}`, { auth: true });
        const list = res?.data || res || [];
        if (!ignore) {
          setParents(Array.isArray(list) ? list : []);
          const map = {};
          list.forEach(p => { map[p._id || p.id] = p; });
          setParentMap(map);
        }
      } catch {
        if (!ignore) { setParents([]); setParentMap({}); }
      }
    })();
    return () => { ignore = true; };
  }, [propertyCode]);
  ...
  const filtered = useMemo(() => {
    ...
    return rows.filter(r =>
      [r.code, r.name, r.description].filter(Boolean)
    ...
    <div className="page" style={{ display: "grid", gridTemplateColumns: "auto 1fr" }}>
      ...
      <div className="res-wrap">
        ...
        <div className="res-topbar">
          <h2 style={{ margin: 0 }}>FNB — Categories</h2>
              ...
              <table className="table">
                ...
                <tbody>
                  {(!dataToRender || dataToRender.length === 0) && !loading && (
                    <tr className="no-rows"><td colSpan={8}>No categories found</td></tr>
                  )}

                  {dataToRender?.map(r => {
                    const id = r._id || r.id;
                    const pId = r.parentId || r.parent?._id || r.parent?.id;
                    const parent = parentMap[pId] || r.parent;
                    const parentName = parent?.name || r.parentName || r.parentCode || "—";
                    return (
                      <tr key={id}>
                        <td>
                        ...
                        <td>{r.name}</td>
                        <td>{parentName}</td>
                        <td>{Number.isFinite(+r.sortOrder) ? +r.sortOrder : "—"}</td>
      ...
      {showDelete && (
        <ConfirmModal
          ...
          onConfirm={async () => {
            ...
            await apiFetch(`/api/fnb/categories/${id}`, { method: "DELETE", auth: true });
            afterDelete(id);
          }}
        />
      )}
...
/* ---------- Form Modal ---------- */
function CategoryFormModal({ initial, defaultPropertyCode, parents, onClose, onSaved }) {
  ...
  const [parentId, setParentId] = useState(initial?.parentId || initial?.parent?._id || initial?.parent?.id || "");
  ...
  const onSubmit = async (e) => {
    ...
    try {
      ...
      if (isEdit) {
        ...
        saved = await apiFetch(`/api/fnb/categories/${id}`, {
          method: "PATCH", auth: true, body: JSON.stringify(payload),
        });
      } else {
        saved = await apiFetch("/api/fnb/categories", {
  ...
  return (
    <Modal title={isEdit ? "Edit Category" : "Create Category"} onClose={onClose}>
      {err && <Banner type="err">{err}</Banner>}
