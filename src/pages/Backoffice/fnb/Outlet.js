// src/pages/Backoffice/fnb/Outlet.js
...
export default function Outlet() {
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      try {
        ...
        const res = await apiFetch(`/api/outlets?${params.toString()}`, { auth: true });
        ...
        if (!ignore) {
          setErr(e?.message || "Failed to load outlets.");
          setRows([]); setTotal(0);
        }
      } finally {
  ...
  const filtered = useMemo(() => {
    ...
    return rows.filter(r =>
      [r.code, r.name, r.type, r.floor, r.defaultKOTPrinter]
        .filter(Boolean)
    ...
    <div className="page" style={{ display: "grid", gridTemplateColumns: "auto 1fr" }}>
      ...
      <div className="res-wrap">
        ...
        <div className="res-topbar">
          <h2 style={{ margin: 0 }}>Outlet</h2>
          <div style={{ display: "flex", gap: 8 }}>
            <input
              className="res-select"
              placeholder="Search (code / name / type / floor / printer)"
              value={q}
            ...
            <button className="btn" onClick={openCreate}>+ Add Outlet</button>
          </div>
        </div>
              ...
              <table className="table">
                ...
                <tbody>
                  {(!dataToRender || dataToRender.length === 0) && !loading && (
                    <tr className="no-rows"><td colSpan={9}>No outlets found</td></tr>
                  )}
      ...
      {showDelete && (
        <ConfirmModal
          title="Delete Outlet?"
          message={`Delete outlet "${toDelete?.name}" (${toDelete?.code})? This cannot be undone.`}
          confirmText="Delete"
          onClose={() => { setShowDelete(false); setToDelete(null); }}
          onConfirm={async () => {
            const id = toDelete?._id || toDelete?.id;
            await apiFetch(`/api/outlets/${id}`, { method: "DELETE", auth: true });
            afterDelete(id);
          }}
        />
      )}
...
/* ---------- Form Modal ---------- */
function OutletFormModal({ initial, defaultPropertyCode, onClose, onSaved }) {
  ...
  const [type, setType] = useState(initial?.type || "DINE_IN"); // DINE_IN / TAKEAWAY / DELIVERY / BAR / OTHER
  const [floor, setFloor] = useState(initial?.floor || "");
  const [defaultKOTPrinter, setDefaultKOTPrinter] = useState(initial?.defaultKOTPrinter || "");
  ...
  const onSubmit = async (e) => {
    ...
    if (!code.trim()) return setErr("Outlet code is required.");
    ...
    const payload = {
      ...
      defaultKOTPrinter: String(defaultKOTPrinter || "").trim(),
      isActive: !!isActive,
    };
    ...
    try {
      ...
      if (isEdit) {
        ...
        saved = await apiFetch(`/api/outlets/${id}`, { method: "PATCH", auth: true, body: JSON.stringify(payload) });
      } else {
        saved = await apiFetch("/api/outlets", { method: "POST", auth: true, body: JSON.stringify(payload) });
      }
      setOk("Saved.");
      onSaved(saved);
    } catch (e2) {
      setErr(e2?.message || "Failed to save outlet.");
    } finally {
      setSaving(false);
    }
  };
      ...
      <form onSubmit={onSubmit} style={{ display: "grid", gap: 12 }}>
        <Row>
          <Field label="Property Code">
            <input className="input" placeholder="Optional scope"
              value={propertyCode}
        ...
        <Row>
          ...
          <Field label="Default KOT Printer">
            <input className="input" value={defaultKOTPrinter} onChange={e => setDefaultKOTPrinter(e.target.value)} />
          </Field>
        </Row>
