...
export default function OutOfStock() {
  ...
  // Client fallback search (when server doesn't filter)
  const filtered = useMemo(() => {
    const term = q.trim().toLowerCase();
    if (!term) return rows;
    return rows.filter(r =>
      [r.itemCode, r.itemName, r.outlet, r.reason]
        .filter(Boolean)
  ...
  return (
    <div className="page" style={{ display: "grid", gridTemplateColumns: "auto 1fr" }}>
      ...
      <div className="res-wrap">
        ...
        <div className="res-topbar">
          ...
          <div style={{ display: "flex", gap: 8, flexWrap: "wrap" }}>
            <input
              ...
              placeholder="Search (item / code / outlet / reason)"
              value={q}
        ...
        <div className="panel">
          <div className="panel-h">
            <span>Items marked Out Of Stock</span>
      ...
      {showDelete && (
        <ConfirmModal
          title="Delete Out-Of-Stock Entry?"
          message={`Remove OOS for "${toDelete?.itemName}" (${toDelete?.itemCode || "no code"})?`}
          confirmText="Delete"
...
/* ---------- Form ---------- */
function OutOfStockForm({ initial, onClose, onSaved }) {
  ...
  const [startDate, setStartDate] = useState(initial?.startDate ? isoDate(initial.startDate) : "");
  const [endDate, setEndDate] = useState(initial?.endDate ? isoDate(initial.endDate) : "");
...
function isoDate(d) {
  // convert ISO or date-like into yyyy-mm-dd for <input type="date">
  const dt = new Date(d);
