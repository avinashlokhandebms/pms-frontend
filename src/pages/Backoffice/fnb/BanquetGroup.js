...
export default function BanquetGroup() {
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      try {
        ...
        const res = await apiFetch(`/api/fnb/banquet-groups?${params.toString()}`, { auth: true });
        ...
        if (!ignore) {
          setErr(e?.message || "Failed to load banquet groups.");
          setRows([]); setTotal(0);
        }
      } finally {
  ...
  const filtered = useMemo(() => {
    ...
    return rows.filter(r =>
      [r.code, r.name, r.hallName, r.location, r.description]
        .filter(Boolean)
    ...
    <div className="page" style={{ display: "grid", gridTemplateColumns: "auto 1fr" }}>
      ...
      <div className="res-wrap">
        ...
        <div className="res-topbar">
          <h2 style={{ margin: 0 }}>Banquet Groups</h2>
          <div style={{ display: "flex", gap: 8, flexWrap: "wrap" }}>
            <input
              className="res-select"
              placeholder="Search (code / name / hall / location)"
              value={q}
            ...
            <button className="btn" onClick={openCreate}>+ Add Group</button>
          </div>
        </div>
              ...
              <table className="table">
                ...
                <tbody>
                  {(!dataToRender || dataToRender.length === 0) && !loading && (
                    <tr className="no-rows"><td colSpan={9}>No groups found</td></tr>
                  )}

                  {dataToRender?.map(r => {
                    const id = r._id || r.id;
                    const cap = (r.capacityMin || r.capacityMax) ? `${r.capacityMin || 0}–${r.capacityMax || 0}` : "—";
                    return (
                      <tr key={id}>
                        <td>
      ...
      {showDelete && (
        <ConfirmModal
          ...
          onConfirm={async () => {
            ...
            await apiFetch(`/api/fnb/banquet-groups/${id}`, { method: "DELETE", auth: true });
            afterDelete(id);
          }}
        />
      )}
...
function GroupFormModal({ initial, onClose, onSaved }) {
  ...
  const onSubmit = async (e) => {
    ...
    const payload = {
      ...
      capacityMin: capacityMin === "" ? null : Number(capacityMin),
    ...
    try {
      ...
      if (isEdit) {
        ...
        saved = await apiFetch(`/api/fnb/banquet-groups/${id}`, {
          method: "PATCH", auth: true, body: JSON.stringify(payload),
        });
      } else {
        saved = await apiFetch("/api/fnb/banquet-groups", {
      ...
      setErr(e2?.message || "Failed to save group.");
    } finally {
      setSaving(false);
    }
  };
  ...
  return (
    <Modal title={isEdit ? "Edit Banquet Group" : "Add Banquet Group"} onClose={onClose}>
      {err && <Banner type="err">{err}</Banner>}
      ...
      <form onSubmit={onSubmit} style={{ display: "grid", gap: 12 }}>
        <Row>
          ...
          <Field label="Hall">
            <input className="input" value={hallName} onChange={e => setHallName(e.target.value)} placeholder="e.g., Ballroom A" />
          </Field>
        </Row>

        <Row>
          <Field label="Location">
            <input className="input" value={location} onChange={e => setLocation(e.target.value)} placeholder="e.g., Ground Floor" />
          </Field>
