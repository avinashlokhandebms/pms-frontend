// src/pages/Backoffice/MealPlans.js
...
export default function MealPlans() {
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      try {
        ...
        const res = await apiFetch(`/api/mealplans?${params.toString()}`, { auth: true });
        ...
        if (!ignore) { setErr(e?.message || "Failed to load meal plans."); setRows([]); setTotal(0); }
      } finally {
  ...
  // remove adult/child from search too
  const filtered = useMemo(() => {
    const term = q.trim().toLowerCase();
    if (!term) return rows;
    return rows.filter((r) =>
      [r.propertyCode, r.code, r.name, r.type, r.description, r.details, r.taxType]
        .filter(Boolean)
    ...
    <div className="page" style={{ display: "grid", gridTemplateColumns: "auto 1fr" }}>
      ...
      <div className="res-wrap">
        ...
        <div className="res-topbar">
          ...
          <div style={{ display: "flex", gap: 8 }}>
            ...
            <input
              className="res-select"
              placeholder="Search (code / name / type / details / tax)"
              value={q}
            ...
            <button className="btn" onClick={openCreate}>+ Add Meal Plan</button>
          </div>
        </div>
              ...
              <table className="table">
                ...
                <tbody>
                  {(!dataToRender || dataToRender.length === 0) && !loading && (
                    <tr className="no-rows"><td colSpan={11}>No meal plans found</td></tr>
                  )}
                  ...
                  {dataToRender?.map(r => {
                    ...
                    return (
                      <tr key={id}>
                        ...
                        <td title={id} style={{ fontFeatureSettings: "'tnum' 1" }}>{String(id).slice(-6)}</td>
                        <td>{r.propertyCode}</td>
                        <td>{r.code}</td>
                        <td>{r.name}</td>
                        <td title={r.details || r.description || ""}>{r.details || r.description || "—"}</td>
                        <td>{(r.taxType || "none").toUpperCase()}</td>
                        <td>{r.type}</td>
      ...
      {showDelete && (
        <ConfirmModal
          ...
          message={`Delete meal plan "${toDelete?.name}" (${toDelete?.code})? This cannot be undone.`}
          confirmText="Delete"
          onClose={() => { setShowDelete(false); setToDelete(null); }}
          onConfirm={async () => {
            const id = toDelete?._id || toDelete?.id;
            await apiFetch(`/api/mealplans/${id}`, { method: "DELETE", auth: true });
            afterDelete(id);
          }}
        />
      )}
...
function MealPlanFormModal({ initial, onClose, onSaved, propsList }) {
  ...
  const [type, setType] = useState(initial?.type || "EP");

  // kept fields
  const [details, setDetails] = useState(initial?.details || initial?.description || "");
  const [taxType, setTaxType] = useState(initial?.taxType || "none");
  ...
  const onSubmit = async (e) => {
    ...
    try {
      ...
      if (isEdit) {
        ...
        saved = await apiFetch(`/api/mealplans/${id}`, { method: "PATCH", auth: true, body: JSON.stringify(payload) });
      } else {
        saved = await apiFetch("/api/mealplans", { method: "POST", auth: true, body: JSON.stringify(payload) });
      }
      setOk("Saved.");
      onSaved(saved);
    } catch (e2) {
      setErr(e2?.message || "Failed to save meal plan.");
    } finally {
      setSaving(false);
    }
  };
  ...
  return (
    <Modal title={isEdit ? "Edit Meal Plan" : "Create Meal Plan"} onClose={onClose}>
      {err && <Banner type="err">{err}</Banner>}
