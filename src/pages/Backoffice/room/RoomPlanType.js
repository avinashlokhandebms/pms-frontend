...
export default function RoomPlanType() {
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      try {
        ...
        const res = await apiFetch(`/api/room/plan-types?${params.toString()}`, { auth: true });
        ...
        if (!ignore) { setErr(e?.message || "Failed to load room plan types."); setRows([]); setTotal(0); }
      } finally {
    ...
    <div className="page" style={{ display: "grid", gridTemplateColumns: "auto 1fr" }}>
      ...
      <div className="res-wrap">
        ...
        <div className="res-topbar">
          <h2 style={{ margin: 0 }}>Room Plan Type</h2>
          <div style={{ display: "flex", gap: 8, flexWrap: "wrap" }}>
            ...
            <button className="btn" onClick={openCreate}>+ Add Plan Type</button>
          </div>
        </div>
              ...
              <table className="table">
                ...
                <tbody>
                  {(!dataToRender || dataToRender.length === 0) && !loading && (
                    <tr className="no-rows"><td colSpan={8}>No plan types found</td></tr>
                  )}
      ...
      {showDelete && (
        <ConfirmModal
          ...
          onConfirm={async () => {
            ...
            await apiFetch(`/api/room/plan-types/${id}`, { method: "DELETE", auth: true });
            afterDelete(id);
          }}
        />
      )}
...
/* ---------- Form Modal ---------- */
function RoomPlanTypeFormModal({ initial, onClose, onSaved, propertyCode }) {
  ...
  const [includeBreakfast, setIncludeBreakfast] = useState(!!initial?.includeBreakfast);
  const [includeLunch, setIncludeLunch] = useState(!!initial?.includeLunch);
  const [includeDinner, setIncludeDinner] = useState(!!initial?.includeDinner);
  ...
  const onSubmit = async (e) => {
    ...
    try {
      ...
      if (isEdit) {
        ...
        saved = await apiFetch(`/api/room/plan-types/${id}`, {
          method: "PATCH", auth: true, body: JSON.stringify(payload),
        });
      } else {
        saved = await apiFetch("/api/room/plan-types", {
      ...
      setErr(e2?.message || "Failed to save room plan type.");
    } finally {
      setSaving(false);
    }
  };
  ...
  return (
    <Modal title={isEdit ? "Edit Room Plan Type" : "Add Room Plan Type"} onClose={onClose}>
      {err && <Banner type="err">{err}</Banner>}
...
/* ---------- helpers/UI ---------- */
function mealsToLabel(r) {
  const arr = [];
  if (r.includeBreakfast) arr.push("B");
  if (r.includeLunch) arr.push("L");
  if (r.includeDinner) arr.push("D");
