...
export default function AmenityCategory() {
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      try {
        ...
        if (!ignore) { setErr(e?.message || "Failed to load amenity categories."); setRows([]); setTotal(0); }
      } finally {
  ...
  const filtered = useMemo(() => {
    ...
    return rows.filter(r =>
      [r.code, r.name, r.description, r.icon]
        .filter(Boolean)
    ...
    <div className="page" style={{ display: "grid", gridTemplateColumns: "auto 1fr" }}>
      ...
      <div className="res-wrap">
        ...
        <div className="res-topbar">
          <h2 style={{ margin: 0 }}>Amenity Category Master</h2>
          <div style={{ display: "flex", gap: 8, flexWrap: "wrap" }}>
            <input
              className="res-select"
              placeholder="Search (code / name / description / icon)"
              value={q}
              ...
              <table className="table">
                ...
                <tbody>
                  {(!dataToRender || dataToRender.length === 0) && !loading && (
                    <tr className="no-rows"><td colSpan={9}>No categories found</td></tr>
                  )}
      ...
      {showDelete && (
        <ConfirmModal
          ...
          onConfirm={async () => {
            ...
            await apiFetch(`/api/room/amenity-categories/${id}`, { method: "DELETE", auth: true });
            afterDelete(id);
          }}
        />
      )}
...
/* ---------- Form Modal ---------- */
function AmenityCategoryFormModal({ initial, onClose, onSaved, propertyCode }) {
  ...
  const onSubmit = async (e) => {
    ...
    try {
      ...
      if (isEdit) {
        ...
        saved = await apiFetch(`/api/room/amenity-categories/${id}`, {
          method: "PATCH", auth: true, body: JSON.stringify(payload),
        });
      } else {
        saved = await apiFetch("/api/room/amenity-categories", {
  ...
  return (
    <Modal title={isEdit ? "Edit Amenity Category" : "Add Amenity Category"} onClose={onClose}>
      {err && <Banner type="err">{err}</Banner>}
      ...
      <form onSubmit={onSubmit} style={{ display: "grid", gap: 12 }}>
        <Row>
          ...
          <Field label="Icon (optional)">
            <input className="input" value={icon} onChange={e => setIcon(e.target.value)} placeholder="e.g. shower, wifi" />
          </Field>
        </Row>
