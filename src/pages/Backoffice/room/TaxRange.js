...
export default function TaxRange() {
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      try {
        ...
        const res = await apiFetch(`/api/room/tax-ranges?${params.toString()}`, { auth: true });
        ...
        if (!ignore) {
          setErr(e?.message || "Failed to load tax ranges.");
          setRows([]); setTotal(0);
        }
      } finally {
    ...
    <div className="page" style={{ display: "grid", gridTemplateColumns: "auto 1fr" }}>
      ...
      <div className="res-wrap">
        ...
        <div className="res-topbar">
          <h2 style={{ margin: 0 }}>Tax Range</h2>
          <div style={{ display: "flex", gap: 8, flexWrap: "wrap" }}>
            ...
            <button className="btn" onClick={openCreate}>+ Add Tax Range</button>
          </div>
        </div>
              ...
              <table className="table">
                ...
                <tbody>
                  {(!dataToRender || dataToRender.length === 0) && !loading && (
                    <tr className="no-rows"><td colSpan={9}>No tax ranges found</td></tr>
                  )}
                  ...
                  {dataToRender?.map(r => {
                    ...
                    return (
                      <tr key={id}>
                        ...
                        <td>{num(r.fromAmount)}</td>
                        <td>{num(r.toAmount)}</td>
      ...
      {showDelete && (
        <ConfirmModal
          title="Delete Tax Range?"
          message={`Delete "${toDelete?.name}" (${toDelete?.code})? This cannot be undone.`}
          confirmText="Delete"
          onClose={() => { setShowDelete(false); setToDelete(null); }}
          onConfirm={async () => {
            const id = toDelete?._id || toDelete?.id;
            await apiFetch(`/api/room/tax-ranges/${id}`, { method: "DELETE", auth: true });
            afterDelete(id);
          }}
        />
      )}
...
/* ---------- Form Modal ---------- */
function TaxRangeFormModal({ initial, onClose, onSaved, propertyCode }) {
  ...
  const [fromAmount, setFromAmount] = useState(initial?.fromAmount ?? "");
  ...
  const onSubmit = async (e) => {
    ...
    const payload = {
      ...
      name: name.trim(),
      fromAmount: fromAmount === "" ? null : Number(fromAmount),
    ...
    try {
      ...
      if (isEdit) {
        ...
        saved = await apiFetch(`/api/room/tax-ranges/${id}`, {
          method: "PATCH", auth: true, body: JSON.stringify(payload),
        });
      } else {
        saved = await apiFetch("/api/room/tax-ranges", {
      ...
      setErr(e2?.message || "Failed to save tax range.");
    } finally {
      setSaving(false);
    }
  };
  ...
  return (
    <Modal title={isEdit ? "Edit Tax Range" : "Add Tax Range"} onClose={onClose}>
      {err && <Banner type="err">{err}</Banner>}
      ...
      <form onSubmit={onSubmit} style={{ display: "grid", gap: 12 }}>
        ...
        <Row>
          <Field label="From Amount">
            <input className="input" type="number" min="0" step="0.01" value={fromAmount} onChange={e => setFromAmount(e.target.value)} />
          </Field>
          <Field label="To Amount">
            <input className="input" type="number" min="0" step="0.01" value={toAmount} onChange={e => setToAmount(e.target.value)} />
          </Field>
          <Field label="Tax %">
            <input className="input" type="number" min="0" max="100" step="0.01" value={taxPercent} onChange={e => setTaxPercent(e.target.value)} />
          </Field>
        </Row>
