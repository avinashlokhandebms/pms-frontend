...
export default function RoomViewType() {
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      try {
        ...
        const res = await apiFetch(`/api/room/view-types?${params.toString()}`, { auth: true });
        ...
        if (!ignore) { setErr(e?.message || "Failed to load room view types."); setRows([]); setTotal(0); }
      } finally {
    ...
    <div className="page" style={{ display: "grid", gridTemplateColumns: "auto 1fr" }}>
      ...
      <div className="res-wrap">
        ...
        <div className="res-topbar">
          <h2 style={{ margin: 0 }}>Room View Type</h2>
          <div style={{ display: "flex", gap: 8, flexWrap: "wrap" }}>
            ...
            <button className="btn" onClick={openCreate}>+ Add View Type</button>
          </div>
        </div>
              ...
              <table className="table">
                ...
                <tbody>
                  {(!dataToRender || dataToRender.length === 0) && !loading && (
                    <tr className="no-rows"><td colSpan={8}>No view types found</td></tr>
                  )}
                  ...
                  {dataToRender?.map(r => {
                    ...
                    return (
                      <tr key={id}>
                        ...
                        <td>{r.code}</td>
                        <td>{r.name}</td>
                        <td>{typeof r.displayOrder === "number" ? r.displayOrder : "—"}</td>
      ...
      {showDelete && (
        <ConfirmModal
          ...
          onConfirm={async () => {
            ...
            await apiFetch(`/api/room/view-types/${id}`, { method: "DELETE", auth: true });
            afterDelete(id);
          }}
        />
      )}
...
/* ---------- Form Modal ---------- */
function ViewTypeFormModal({ initial, onClose, onSaved, propertyCode }) {
  ...
  const [displayOrder, setDisplayOrder] = useState(
    typeof initial?.displayOrder === "number" ? initial.displayOrder : ""
  );
  ...
  const onSubmit = async (e) => {
    ...
    try {
      ...
      if (isEdit) {
        ...
        saved = await apiFetch(`/api/room/view-types/${id}`, {
          method: "PATCH", auth: true, body: JSON.stringify(payload),
        });
      } else {
        saved = await apiFetch("/api/room/view-types", {
          method: "POST", auth: true, body: JSON.stringify(payload),
        });
      }
      setOk("Saved."); onSaved(saved);
    } catch (e2) {
      setErr(e2?.message || "Failed to save view type.");
    } finally {
      setSaving(false);
    }
  };
  ...
  return (
    <Modal title={isEdit ? "Edit Room View Type" : "Add Room View Type"} onClose={onClose}>
      {err && <Banner type="err">{err}</Banner>}
