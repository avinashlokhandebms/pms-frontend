...
export default function BedSize() {
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      try {
        ...
        const res = await apiFetch(`/api/room/bed-sizes?${params.toString()}`, { auth: true });
        ...
        if (!ignore) { setErr(e?.message || "Failed to load bed sizes."); setRows([]); setTotal(0); }
      } finally {
  ...
  return (
    <div className="page" style={{ display: "grid", gridTemplateColumns: "auto 1fr" }}>
      ...
      <div className="res-wrap">
        ...
        <div className="res-topbar">
          ...
          <div style={{ display: "flex", gap: 8, flexWrap: "wrap" }}>
            ...
            <button className="btn" onClick={openCreate}>+ Add Bed Size</button>
          </div>
        </div>
              ...
              <table className="table">
                ...
                <tbody>
                  {(!dataToRender || dataToRender.length === 0) && !loading && (
                    <tr className="no-rows"><td colSpan={8}>No bed sizes found</td></tr>
                  )}

                  {dataToRender?.map(r => {
                    const id = r._id || r.id;
                    const wl = [
                      (typeof r.widthIn === "number" && r.widthIn > 0) ? r.widthIn : null,
                      (typeof r.lengthIn === "number" && r.lengthIn > 0) ? r.lengthIn : null
                    ];
                    const wlStr = wl.every(n => n !== null) ? `${wl[0]}×${wl[1]}` : "—";
                    return (
                      <tr key={id}>
                        <td>
      ...
      {showDelete && (
        <ConfirmModal
          title="Delete Bed Size?"
          message={`Delete "${toDelete?.name}" (${toDelete?.code})? This cannot be undone.`}
          confirmText="Delete"
          onClose={() => { setShowDelete(false); setToDelete(null); }}
          onConfirm={async () => {
            const id = toDelete?._id || toDelete?.id;
            await apiFetch(`/api/room/bed-sizes/${id}`, { method: "DELETE", auth: true });
            afterDelete(id);
          }}
        />
      )}
...
/* ---------- Form Modal ---------- */
function BedSizeFormModal({ initial, onClose, onSaved, propertyCode }) {
  ...
  const [widthIn, setWidthIn] = useState(
    typeof initial?.widthIn === "number" ? initial.widthIn : ""
  );
  const [lengthIn, setLengthIn] = useState(
    typeof initial?.lengthIn === "number" ? initial.lengthIn : ""
  );
  ...
  const onSubmit = async (e) => {
    ...
    const payload = {
      ...
      isActive: !!isActive,
      widthIn: widthIn === "" ? undefined : Number(widthIn),
      lengthIn: lengthIn === "" ? undefined : Number(lengthIn),
      ...(propertyCode ? { propertyCode } : {}),
    };
    ...
    try {
      ...
      if (isEdit) {
        ...
        saved = await apiFetch(`/api/room/bed-sizes/${id}`, {
          method: "PATCH", auth: true, body: JSON.stringify(payload),
        });
      } else {
        saved = await apiFetch("/api/room/bed-sizes", {
          method: "POST", auth: true, body: JSON.stringify(payload),
        });
      }
      setOk("Saved."); onSaved(saved);
    } catch (e2) {
      setErr(e2?.message || "Failed to save bed size.");
    } finally {
      setSaving(false);
    }
  };
  ...
  return (
    <Modal title={isEdit ? "Edit Bed Size" : "Add Bed Size"} onClose={onClose}>
      {err && <Banner type="err">{err}</Banner>}
