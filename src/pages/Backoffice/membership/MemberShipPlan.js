  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      try {
        ...
        const res = await apiFetch(`/api/membership-plans?${params.toString()}`, { auth: true });
        ...
        if (!ignore) {
          setErr(e?.message || "Failed to load membership plans.");
          setRows([]); setTotal(0);
        }
      } finally {
    ...
    <div className="page" style={{ display: "grid", gridTemplateColumns: "auto 1fr" }}>
      ...
      <div className="res-wrap">
        ...
        <div className="res-topbar">
          <div>
            <h2 style={{ margin: 0 }}>MemberShip Plan</h2>
              ...
              <table className="table">
                <thead>
                  <tr>
                    ...
                    <th>Name</th>
                    <th>Description</th>
                    <th style={{ textAlign: "right" }}>Validity (days)</th>
                    <th style={{ textAlign: "right" }}>Max Visits</th>
                ...
                <tbody>
                  {(!dataToRender || dataToRender.length === 0) && !loading && (
                    <tr className="no-rows"><td colSpan={11}>No membership plans found</td></tr>
                  )}
                  ...
                  {dataToRender?.map(r => {
                    ...
                    return (
                      <tr key={id}>
                        ...
                        <td style={{ textAlign: "right" }}>{r.validityDays ?? "—"}</td>
                        <td style={{ textAlign: "right" }}>{r.maxVisits ?? "—"}</td>
                        <td style={{ textAlign: "right" }}>{fmtMoney(r.price)}</td>
                        <td style={{ textAlign: "right" }}>{Number(r.taxRatePct ?? 0).toFixed(2)}</td>
      ...
      {showDelete && (
        <ConfirmModal
          ...
          onConfirm={async () => {
            ...
            await apiFetch(`/api/membership-plans/${id}`, {
              method: "DELETE", auth: true, body: JSON.stringify({ propertyCode })
            });
            afterDelete(id);
          }}
        />
...
/* ---------- Form Modal ---------- */
function PlanFormModal({ initial, onClose, onSaved, propertyCode }) {
  ...
  const [validityDays, setValidityDays] = useState(initial?.validityDays ?? 365);
  const [maxVisits, setMaxVisits] = useState(initial?.maxVisits ?? 0); // 0 = unlimited
  ...
  const onSubmit = async (e) => {
    ...
    try {
      ...
      if (isEdit) {
        ...
        saved = await apiFetch(`/api/membership-plans/${id}`, {
          method: "PATCH", auth: true, body: JSON.stringify(payload),
        });
      } else {
        saved = await apiFetch("/api/membership-plans", {
      ...
      setErr(e2?.message || "Failed to save membership plan.");
    } finally {
      setSaving(false);
    }
  };
  ...
  return (
    <Modal title={isEdit ? "Edit MemberShip Plan" : "Create MemberShip Plan"} onClose={onClose}>
      {err && <Banner type="err">{err}</Banner>}
      ...
      <form onSubmit={onSubmit} style={{ display: "grid", gap: 12 }}>
        ...
        <Row>
          <Field label="Validity (days)">
            <input className="input" type="number" min="0" step="1" value={validityDays} onChange={e => setValidityDays(e.target.value)} />
          </Field>
          <Field label="Max Visits (0 = unlimited)">
            <input className="input" type="number" min="0" step="1" value={maxVisits} onChange={e => setMaxVisits(e.target.value)} />
          </Field>
          <Field label="Price">
        ...
        <Row>
          <Field label="Tax %">
            <input className="input" type="number" min="0" step="0.01" value={taxRatePct} onChange={e => setTaxRatePct(e.target.value)} />
          </Field>
        </Row>
