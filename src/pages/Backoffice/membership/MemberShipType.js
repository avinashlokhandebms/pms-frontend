  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      try {
        ...
        const res = await apiFetch(`/api/membership-types?${params.toString()}`, { auth: true });
        ...
        if (!ignore) {
          setErr(e?.message || "Failed to load membership types.");
          setRows([]); setTotal(0);
        }
      } finally {
    ...
    <div className="page" style={{ display: "grid", gridTemplateColumns: "auto 1fr" }}>
      ...
      <div className="res-wrap">
        ...
        <div className="res-topbar">
          <div>
            <h2 style={{ margin: 0 }}>MemberShip Type</h2>
              ...
              <table className="table">
                <thead>
                  <tr>
                    ...
                    <th style={{ textAlign: "right" }}>Joining Fee</th>
                    <th style={{ textAlign: "right" }}>Renewal Fee</th>
                    <th>Active</th>
                    <th>Created</th>
                    <th>Updated</th>
                  </tr>
                ...
                <tbody>
                  {(!dataToRender || dataToRender.length === 0) && !loading && (
                    <tr className="no-rows"><td colSpan={10}>No membership types found</td></tr>
                  )}
                  ...
                  {dataToRender?.map(r => {
                    ...
                    return (
                      <tr key={id}>
                        ...
                        <td style={{ textAlign: "right" }}>{fmtMoney(r.joiningFee)}</td>
                        <td style={{ textAlign: "right" }}>{fmtMoney(r.renewalFee)}</td>
      ...
      {showDelete && (
        <ConfirmModal
          ...
          onConfirm={async () => {
            ...
            await apiFetch(`/api/membership-types/${id}`, { method: "DELETE", auth: true, body: JSON.stringify({ propertyCode }) });
            afterDelete(id);
          }}
        />
      )}
...
/* ---------- Form Modal ---------- */
function TypeFormModal({ initial, onClose, onSaved, propertyCode }) {
  ...
  const [joiningFee, setJoiningFee] = useState(initial?.joiningFee ?? 0);
  const [renewalFee, setRenewalFee] = useState(initial?.renewalFee ?? 0);
  ...
  const onSubmit = async (e) => {
    ...
    try {
      ...
      if (isEdit) {
        ...
        saved = await apiFetch(`/api/membership-types/${id}`, {
          method: "PATCH", auth: true, body: JSON.stringify(payload),
        });
      } else {
        saved = await apiFetch("/api/membership-types", {
      ...
      setErr(e2?.message || "Failed to save membership type.");
    } finally {
      setSaving(false);
    }
  };
  ...
  return (
    <Modal title={isEdit ? "Edit MemberShip Type" : "Create MemberShip Type"} onClose={onClose}>
      {err && <Banner type="err">{err}</Banner>}
      ...
      <form onSubmit={onSubmit} style={{ display: "grid", gap: 12 }}>
        ...
        <Row>
          <Field label="Validity (days)">
            <input className="input" type="number" min="0" value={validityDays} onChange={e => setValidityDays(e.target.value)} />
          </Field>
          <Field label="Joining Fee">
            <input className="input" type="number" min="0" step="0.01" value={joiningFee} onChange={e => setJoiningFee(e.target.value)} />
          </Field>
          <Field label="Renewal Fee">
            <input className="input" type="number" min="0" step="0.01" value={renewalFee} onChange={e => setRenewalFee(e.target.value)} />
          </Field>
        </Row>
