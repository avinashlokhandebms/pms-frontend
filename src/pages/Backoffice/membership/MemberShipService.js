  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      try {
        ...
        const res = await apiFetch(`/api/membership-services?${params.toString()}`, { auth: true });
        ...
        if (!ignore) {
          setErr(e?.message || "Failed to load membership services.");
          setRows([]); setTotal(0);
        }
      } finally {
    ...
    <div className="page" style={{ display: "grid", gridTemplateColumns: "auto 1fr" }}>
      ...
      <div className="res-wrap">
        ...
        <div className="res-topbar">
          <div>
            <h2 style={{ margin: 0 }}>MemberShip Service</h2>
          ...
          <div style={{ display: "flex", gap: 8 }}>
            ...
            <button className="btn" onClick={openCreate}>+ Add Service</button>
          </div>
        </div>
              ...
              <table className="table">
                <thead>
                  <tr>
                    ...
                    <th>Name</th>
                    <th>Description</th>
                    <th style={{ textAlign: "right" }}>Duration (mins)</th>
                ...
                <tbody>
                  {(!dataToRender || dataToRender.length === 0) && !loading && (
                    <tr className="no-rows"><td colSpan={10}>No membership services found</td></tr>
                  )}
                  ...
                  {dataToRender?.map(r => {
                    ...
                    return (
                      <tr key={id}>
                        ...
                        <td style={{ textAlign: "right" }}>{r.durationMinutes ?? "—"}</td>
      ...
      {showDelete && (
        <ConfirmModal
          title="Delete Membership Service?"
          message={`Delete "${toDelete?.name}" (${toDelete?.code})? This cannot be undone.`}
          confirmText="Delete"
          onClose={() => { setShowDelete(false); setToDelete(null); }}
          onConfirm={async () => {
            const id = toDelete?._id || toDelete?.id;
            await apiFetch(`/api/membership-services/${id}`, { method: "DELETE", auth: true, body: JSON.stringify({ propertyCode }) });
            afterDelete(id);
          }}
        />
      )}
...
/* ---------- Form Modal ---------- */
function ServiceFormModal({ initial, onClose, onSaved, propertyCode }) {
  ...
  const onSubmit = async (e) => {
    ...
    try {
      ...
      if (isEdit) {
        ...
        saved = await apiFetch(`/api/membership-services/${id}`, {
          method: "PATCH", auth: true, body: JSON.stringify(payload),
        });
      } else {
        saved = await apiFetch("/api/membership-services", {
      ...
      setErr(e2?.message || "Failed to save membership service.");
    } finally {
      setSaving(false);
    }
  };
  ...
  return (
    <Modal title={isEdit ? "Edit MemberShip Service" : "Create MemberShip Service"} onClose={onClose}>
      {err && <Banner type="err">{err}</Banner>}
      ...
      <form onSubmit={onSubmit} style={{ display: "grid", gap: 12 }}>
        ...
        <Row>
          <Field label="Duration (mins)">
            <input className="input" type="number" min="0" step="1" value={durationMinutes} onChange={e => setDurationMinutes(e.target.value)} />
          </Field>
          <Field label="Price">
