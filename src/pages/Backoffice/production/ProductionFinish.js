...
export default function ProductionFinish() {
  ...
  // Load list (server paging if available)
  useEffect(() => {
    let ignore = false;
    (async () => {
      setLoading(true);
      ...
      try {
        ...
        const res = await apiFetch(`/api/production-finish?${params.toString()}`, { auth: true });
        ...
        if (!ignore) { setErr(e?.message || "Failed to load production finish records."); setRows([]); setTotal(0); }
      } finally {
  ...
  const filtered = useMemo(() => {
    ...
    return rows.filter(r =>
      [r.docNo, r.item, r.batchNo, r.status, r.notes]
        .filter(Boolean)
    ...
    <div className="page" style={{ display: "grid", gridTemplateColumns: "auto 1fr" }}>
      ...
      <div className="res-wrap">
        ...
        <div className="res-topbar">
          <div>
            <h2 style={{ margin: 0 }}>Production Finish</h2>
          ...
          <div style={{ display: "flex", gap: 8 }}>
            <input
              ...
              placeholder="Search (doc no / item / batch / status / notes)"
              value={q}
        ...
        <div className="panel">
          <div className="panel-h">
            <span>Finished Productions</span>
      ...
      {showDelete && (
        <ConfirmModal
          title="Delete record?"
          message={`Delete production finish "${toDelete?.docNo}"? This cannot be undone.`}
          confirmText="Delete"
          onClose={() => { setShowDelete(false); setToDelete(null); }}
          onConfirm={async () => {
            const id = toDelete?._id || toDelete?.id;
            await apiFetch(`/api/production-finish/${id}`, { method: "DELETE", auth: true });
            afterDelete(id);
          }}
        />
      )}
...
/* ---------- Form Modal ---------- */
function FinishFormModal({ initial, onClose, onSaved, propertyCode }) {
  ...
  const onSubmit = async (e) => {
    ...
    try {
      ...
      if (isEdit) {
        ...
        saved = await apiFetch(`/api/production-finish/${id}`, { method: "PATCH", auth: true, body: JSON.stringify(payload) });
      } else {
        saved = await apiFetch("/api/production-finish", { method: "POST", auth: true, body: JSON.stringify(payload) });
      }
      setOk("Saved.");
      onSaved(saved);
    } catch (e2) {
  ...
  return (
    <Modal title={isEdit ? "Edit Production Finish" : "Add Production Finish"} onClose={onClose}>
      {err && <Banner type="err">{err}</Banner>}
        ...
        <Row>
          ...
          <Field label="Status">
            <select className="res-select" value={status} onChange={e => setStatus(e.target.value)}>
              {["FINISHED", "PENDING", "CANCELLED"].map(s => <option key={s} value={s}>{s}</option>)}
            </select>
          </Field>
        </Row>
...
function StatusPill({ value }) {
  ...
  const map = {
    FINISHED: { bg: "#ecfdf5", bd: "#a7f3d0", fg: "#065f46", txt: "Finished" },
    PENDING:  { bg: "#fefce8", bd: "#fde68a", fg: "#92400e", txt: "Pending" },
