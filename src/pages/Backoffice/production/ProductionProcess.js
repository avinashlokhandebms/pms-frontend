  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      try {
        ...
        const res = await apiFetch(`/api/production-processes?${params.toString()}`, { auth: true });
        ...
        if (!ignore) { setErr(e?.message || "Failed to load processes."); setRows([]); setTotal(0); }
      } finally {
    ...
    <div className="page" style={{ display: "grid", gridTemplateColumns: "auto 1fr" }}>
      ...
      <div className="res-wrap">
        ...
        <div className="res-topbar">
          <div>
            <h2 style={{ margin: 0 }}>Production Processes</h2>
              ...
              <table className="table">
                ...
                <tbody>
                  {(!dataToRender || dataToRender.length === 0) && !loading && (
                    <tr className="no-rows"><td colSpan={8}>No processes found</td></tr>
                  )}
                  ...
                  {dataToRender?.map((r) => {
                    ...
                    return (
                      <tr key={id}>
                        ...
                        <td>{Number(r.stdDurationMin ?? 0)}</td>
      ...
      {showDelete && (
        <ConfirmModal
          ...
          message={`Delete process "${toDelete?.name}" (${toDelete?.code})? This cannot be undone.`}
          confirmText="Delete"
          onClose={() => { setShowDelete(false); setToDelete(null); }}
          onConfirm={async () => {
            const id = toDelete?._id || toDelete?.id;
            await apiFetch(`/api/production-processes/${id}`, { method: "DELETE", auth: true });
            afterDelete(id);
          }}
        />
      )}
...
/* ---------- Form Modal ---------- */
function ProcessFormModal({ initial, onClose, onSaved, propertyCode }) {
  ...
  const [stdDurationMin, setStdDurationMin] = useState(initial?.stdDurationMin ?? 0);
  ...
  const onSubmit = async (e) => {
    ...
    const payload = {
      ...
      stdDurationMin: Number(stdDurationMin || 0),
      isActive: !!isActive,
      propertyCode: propertyCode || "",
    };
    ...
    try {
      ...
      if (isEdit) {
        ...
        saved = await apiFetch(`/api/production-processes/${id}`, {
          method: "PATCH", auth: true, body: JSON.stringify(payload),
        });
      } else {
        saved = await apiFetch("/api/production-processes", {
      ...
      setErr(e2?.message || "Failed to save process.");
    } finally {
      setSaving(false);
    }
  };
  ...
  return (
    <Modal title={isEdit ? "Edit Process" : "Create Process"} onClose={onClose}>
      {err && <Banner type="err">{err}</Banner>}
      ...
      <form onSubmit={onSubmit} style={{ display: "grid", gap: 12 }}>
        ...
        <Row>
          <Field label="Std Duration (minutes)">
            <input
              className="input"
              type="number"
              min="0"
              step="1"
              value={stdDurationMin}
              onChange={e => setStdDurationMin(e.target.value)}
            />
          </Field>
        </Row>
