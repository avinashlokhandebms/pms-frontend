...
export default function Items() {
  ...
  // load ref data (categories, groups, vegTypes)
  useEffect(() => {
    (async () => {
      try {
        const [cRes, gRes, vRes] = await Promise.allSettled([
          apiFetch("/api/fnb/categories?limit=500", { auth: true }),
          apiFetch("/api/fnb/menu-groups?limit=500", { auth: true }),
          apiFetch("/api/common/veg-types?limit=500", { auth: true })
        ]);
        setCats(Array.isArray(cRes.value?.data) ? cRes.value.data : Array.isArray(cRes.value) ? cRes.value : []);
        setGroups(Array.isArray(gRes.value?.data) ? gRes.value.data : Array.isArray(gRes.value) ? gRes.value : []);
        setVegTypes(Array.isArray(vRes.value?.data) ? vRes.value.data : Array.isArray(vRes.value) ? vRes.value : []);
      } catch {
        // fallbacks
        setCats([{ _id: "c1", name: "Starters" }, { _id: "c2", name: "Mains" }]);
        setGroups([{ _id: "g1", name: "Indian" }, { _id: "g2", name: "Chinese" }]);
        setVegTypes([{ _id: "v1", code: "VEG", name: "Veg" }, { _id: "v2", code: "NVG", name: "Non-Veg" }]);
      }
    })();
  }, []);
  ...
  const filtered = useMemo(() => {
    ...
    return rows.filter(r =>
      [r.code, r.name, r.categoryName, r.groupName, r.vegTypeName]
        .filter(Boolean)
        ...
        <div className="res-topbar">
          ...
          <div style={{ display: "flex", gap: 8 }}>
            <input
              ...
              placeholder="Search (code / name / category / group)"
              value={q}
                  ...
                  {dataToRender?.map(r => {
                    ...
                    return (
                      <tr key={id}>
                        ...
                        <td>{r.code}</td>
                        <td>{r.name}</td>
                        <td>{r.categoryName || lookupName(cats, r.categoryId) || "—"}</td>
                        <td>{r.groupName || lookupName(groups, r.groupId) || "—"}</td>
                        <td>{r.vegTypeName || lookupVeg(vegTypes, r.vegType) || "—"}</td>
                        <td style={{ textAlign: "right" }}>{fmtAmt(r.price)}</td>
                        <td style={{ textAlign: "right" }}>{fmtPct(r.taxPct)}</td>
...
/* ---------- Form Modal ---------- */
function ItemFormModal({ initial, onClose, onSaved, cats, groups, vegTypes }) {
  ...
  const [vegType, setVegType] = useState(initial?.vegType || "");
  ...
  const onSubmit = async (e) => {
    ...
    try {
      ...
      if (isEdit) {
        ...
        saved = await apiFetch(`/api/fnb/items/${id}`, { method: "PATCH", auth: true, body: JSON.stringify(payload) });
      } else {
        saved = await apiFetch("/api/fnb/items", { method: "POST", auth: true, body: JSON.stringify(payload) });
      }
      setOk("Saved.");
      onSaved(saved);
    } catch (e2) {
    ...
    <Modal title={isEdit ? "Edit Item" : "Create Item"} onClose={onClose}>
      ...
      <form onSubmit={onSubmit} style={{ display: "grid", gap: 12 }}>
        ...
        <Row>
          ...
          <Field label="Veg Type">
            <select className="res-select" value={vegType} onChange={e => setVegType(e.target.value)}>
              <option value="">—</option>
              {vegTypes.map(v => (
                <option key={v._id || v.id || v.code} value={v.code || v._id || v.id}>
                  {v.name || v.code}
                </option>
              ))}
            </select>
        ...
        <Row>
          ...
          <Field label="Tax %">
            <input className="input" type="number" min="0" step="0.01" value={taxPct} onChange={e => setTaxPct(e.target.value)} />
          </Field>
          <Field label="HSN">
            <input className="input" value={hsn} onChange={e => setHsn(e.target.value)} placeholder="Optional" />
          </Field>
        </Row>
...
function lookupName(list, id) {
  const x = list.find(i => (i._id || i.id) === id);
  return x?.name || "";
}
function lookupVeg(list, code) {
  const x = list.find(i => i.code === code || (i._id || i.id) === code);
...
function fmtPct(n) { const v = Number(n) || 0; return v.toFixed(2); }
