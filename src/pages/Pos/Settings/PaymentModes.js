// src/pages/Backoffice/common/PaymentModes.js
...
const TYPES = ["CASH", "CARD", "UPI", "WALLET", "BANK_TRANSFER", "CREDIT", "OTHER"];
const CHANNELS = ["OFFLINE", "ONLINE", "BOTH"];
...
export default function PaymentModes() {
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      try {
        ...
        const res = await apiFetch(`/api/payment-modes?${params.toString()}`, { auth: true });
        ...
        if (!ignore) {
          setErr(e?.message || "Failed to load payment modes.");
          setRows([]); setTotal(0);
        }
      } finally {
  ...
  const filtered = useMemo(() => {
    ...
    return rows.filter(r =>
      [r.code, r.name, r.type, r.channel, r.ledgerAccount, r.description]
        .filter(Boolean)
    ...
    <div className="page" style={{ display: "grid", gridTemplateColumns: "auto 1fr" }}>
      ...
      <div className="res-wrap">
        ...
        <div className="res-topbar">
          <h2 style={{ margin: 0 }}>Payment Modes</h2>
          <div style={{ display: "flex", gap: 8, alignItems: "center" }}>
            <input
              className="res-select"
              placeholder="Search (code / name / type / channel / ledger)"
              value={q}
            ...
            <button className="btn" onClick={openCreate}>+ Add Payment Mode</button>
          </div>
        </div>
        ...
        <div className="panel">
          <div className="panel-h">
            <span>Configured Payment Modes {propertyCode ? `— ${propertyCode}` : ""}</span>
              ...
              <table className="table">
                ...
                <tbody>
                  {(!dataToRender || dataToRender.length === 0) && !loading && (
                    <tr className="no-rows"><td colSpan={12}>No payment modes found</td></tr>
                  )}
                  ...
                  {dataToRender?.map(r => {
                    ...
                    return (
                      <tr key={id}>
                        ...
                        <td>{r.type}</td>
                        <td>{r.channel || "—"}</td>
                        <td title={r.ledgerAccount}>{r.ledgerAccount || "—"}</td>
                        <td style={{ textAlign: "right" }}>{fmtPct(r.surchargePct)}</td>
                        <td><OnOff value={r.isRefundable} /></td>
      ...
      {showDelete && (
        <ConfirmModal
          ...
          onConfirm={async () => {
            ...
            await apiFetch(`/api/payment-modes/${id}`, { method: "DELETE", auth: true });
            afterDelete(id);
          }}
        />
      )}
...
/* ---------- Form Modal ---------- */
function PaymentModeFormModal({ initial, onClose, onSaved }) {
  ...
  const [channel, setChannel] = useState(initial?.channel || "OFFLINE");
  const [ledgerAccount, setLedgerAccount] = useState(initial?.ledgerAccount || "");
  const [surchargePct, setSurchargePct] = useState(initial?.surchargePct ?? 0);
  ...
  const onSubmit = async (e) => {
    ...
    if (!TYPES.includes(type)) return setErr("Invalid type");
    if (!CHANNELS.includes(channel)) return setErr("Invalid channel");

    const pct = Number(surchargePct || 0);
    if (pct < 0 || pct > 100) return setErr("Surcharge % must be between 0 and 100");
    ...
    try {
      ...
      if (isEdit) {
        ...
        saved = await apiFetch(`/api/payment-modes/${id}`, { method: "PATCH", auth: true, body: JSON.stringify(payload) });
      } else {
        saved = await apiFetch("/api/payment-modes", { method: "POST", auth: true, body: JSON.stringify(payload) });
      }
      setOk("Saved.");
      onSaved(saved);
    } catch (e2) {
      setErr(e2?.message || "Failed to save payment mode.");
    } finally {
      setSaving(false);
    }
  };
  ...
  return (
    <Modal title={isEdit ? "Edit Payment Mode" : "Create Payment Mode"} onClose={onClose}>
      {err && <Banner type="err">{err}</Banner>}
      ...
      <form onSubmit={onSubmit} style={{ display: "grid", gap: 12 }}>
        ...
        <Row>
          <Field label="Channel">
            <select className="res-select" value={channel} onChange={e => setChannel(e.target.value)}>
              {CHANNELS.map(c => <option key={c}>{c}</option>)}
            </select>
          </Field>
          <Field label="Ledger Account">
            <input className="input" placeholder="e.g. 1-200 Sales Clearing" value={ledgerAccount} onChange={e => setLedgerAccount(e.target.value)} />
          </Field>
          <Field label="Surcharge %">
            <input className="input" type="number" min="0" max="100" step="0.01" value={surchargePct} onChange={e => setSurchargePct(e.target.value)} />
          </Field>
        </Row>
