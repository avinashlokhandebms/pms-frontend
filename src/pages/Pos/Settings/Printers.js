// src/pages/Backoffice/common/Printers.js
...
const TYPES = ["RECEIPT", "KOT", "BAR", "KITCHEN", "OTHER"];
const CONNECTIONS = ["NETWORK", "USB", "BLUETOOTH"];
const WIDTHS = [80, 58];
...
export default function Printers() {
  ...
  const filtered = useMemo(() => {
    ...
    return rows.filter(r =>
      [r.code, r.name, r.type, r.connectionType, r.ipAddress, r.devicePath, r.macAddress, r.location]
        .filter(Boolean)
      ...
      <div className="res-wrap">
        ...
        <div className="res-topbar">
          ...
          <div style={{ display: "flex", gap: 8, alignItems: "center" }}>
            <input
              ...
              placeholder="Search (code / name / type / IP / path)"
              value={q}
        ...
        <div className="panel">
          <div className="panel-h">
            <span>Configured Printers {propertyCode ? `— ${propertyCode}` : ""}</span>
              ...
              <table className="table">
                ...
                <tbody>
                  ...
                  {dataToRender?.map(r => {
                    const id = r._id || r.id;
                    const addr =
                      r.connectionType === "NETWORK" ? (r.ipAddress || "—") :
                      r.connectionType === "USB"     ? (r.devicePath || "—") :
                      r.connectionType === "BLUETOOTH" ? (r.macAddress || "—") :
                      "—";
                    return (
                      <tr key={id}>
                        ...
                        <td style={{ textAlign: "right" }}>{r.port ?? (r.connectionType === "NETWORK" ? 9100 : "—")}</td>
                        <td style={{ textAlign: "right" }}>{r.widthMM || "—"}</td>
...
function PrinterFormModal({ initial, onClose, onSaved }) {
  ...
  const [connectionType, setConnectionType] = useState(initial?.connectionType || "NETWORK");
  const [ipAddress, setIpAddress] = useState(initial?.ipAddress || "");
  const [port, setPort] = useState(initial?.port ?? 9100);
  const [devicePath, setDevicePath] = useState(initial?.devicePath || "");
  const [macAddress, setMacAddress] = useState(initial?.macAddress || "");
  const [widthMM, setWidthMM] = useState(initial?.widthMM || 80);
  const [charactersPerLine, setCharactersPerLine] = useState(initial?.charactersPerLine || 48);
  ...
  const onSubmit = async (e) => {
    ...
    const payload = {
      ...
      type,
      connectionType,
      ipAddress: connectionType === "NETWORK" ? (ipAddress || "").trim() : "",
      port: connectionType === "NETWORK" ? Number(port || 9100) : undefined,
      devicePath: connectionType === "USB" ? (devicePath || "").trim() : "",
      macAddress: connectionType === "BLUETOOTH" ? (macAddress || "").trim() : "",
      widthMM: Number(widthMM || 80),
      charactersPerLine: Number(charactersPerLine || 48),
      location,
      description,
      isDefault: !!isDefault,
      isActive: !!isActive,
    ...
    try {
      ...
      if (isEdit) {
        ...
        saved = await apiFetch(`/api/printers/${id}`, { method: "PATCH", auth: true, body: JSON.stringify(payload) });
      } else {
        saved = await apiFetch("/api/printers", { method: "POST", auth: true, body: JSON.stringify(payload) });
      }
      setOk("Saved.");
      onSaved(saved);
    } catch (e2) {
  ...
  return (
    <Modal title={isEdit ? "Edit Printer" : "Create Printer"} onClose={onClose}>
      {err && <Banner type="err">{err}</Banner>}
      ...
      <form onSubmit={onSubmit} style={{ display: "grid", gap: 12 }}>
        ...
        <Row>
          <Field label="Connection">
            <select className="res-select" value={connectionType} onChange={e => setConnectionType(e.target.value)}>
          ...
          {connectionType === "NETWORK" && (
            <>
              <Field label="IP Address">
                <input className="input" placeholder="e.g. 192.168.1.90" value={ipAddress} onChange={e => setIpAddress(e.target.value)} />
              </Field>
              <Field label="Port">
          ...
          {connectionType === "USB" && (
            <>
              <Field label="Device Path">
                <input className="input" placeholder="/dev/usb/lp0 or COM3" value={devicePath} onChange={e => setDevicePath(e.target.value)} />
              </Field>
              <div />
              <div />
            </>
          ...
          {connectionType === "BLUETOOTH" && (
            <>
              <Field label="MAC Address">
                <input className="input" placeholder="e.g. 00:1A:7D:DA:71:13" value={macAddress} onChange={e => setMacAddress(e.target.value)} />
              </Field>
              <div />
              <div />
            </>
        ...
        <Row>
          <Field label="Paper Width (mm)">
            <select className="res-select" value={widthMM} onChange={e => setWidthMM(e.target.value)}>
              {WIDTHS.map(w => <option key={w} value={w}>{w}</option>)}
            </select>
          </Field>
          <Field label="Chars / Line">
            <input className="input" type="number" min="1" value={charactersPerLine} onChange={e => setCharactersPerLine(e.target.value)} />
          </Field>
          <Field label="Location">
            <input className="input" placeholder="e.g. Front Desk / Kitchen" value={location} onChange={e => setLocation(e.target.value)} />
          </Field>
        </Row>
