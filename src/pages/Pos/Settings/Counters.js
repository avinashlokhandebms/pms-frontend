// src/pages/Backoffice/common/Counters.js
...
const TYPES = ["POS", "RECEPTION", "KITCHEN", "BAR", "SERVICE", "OTHER"];
...
export default function Counters() {
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      try {
        ...
        const res = await apiFetch(`/api/counters?${params.toString()}`, { auth: true });
  ...
  const filtered = useMemo(() => {
    ...
    return rows.filter(r =>
      [r.code, r.name, r.type, r.location, r.description]
        .filter(Boolean)
      ...
      <div className="res-wrap">
        ...
        <div className="res-topbar">
          ...
          <div style={{ display: "flex", gap: 8, alignItems: "center" }}>
            ...
            <button className="btn" onClick={openCreate}>+ Add Counter</button>
          </div>
        </div>
        ...
        <div className="panel">
          <div className="panel-h">
            <span>Configured Counters {propertyCode ? `— ${propertyCode}` : ""}</span>
              ...
              <table className="table">
                ...
                <tbody>
                  {(!dataToRender || dataToRender.length === 0) && !loading && (
                    <tr className="no-rows"><td colSpan={10}>No counters found</td></tr>
                  )}
      ...
      {showDelete && (
        <ConfirmModal
          ...
          onConfirm={async () => {
            ...
            await apiFetch(`/api/counters/${id}`, { method: "DELETE", auth: true });
            afterDelete(id);
          }}
        />
      )}
...
/* ---------- Form Modal ---------- */
function CounterFormModal({ initial, onClose, onSaved }) {
  ...
  const [type, setType] = useState(initial?.type || "POS");
  ...
  const onSubmit = async (e) => {
    ...
    try {
      ...
      if (isEdit) {
        ...
        saved = await apiFetch(`/api/counters/${id}`, { method: "PATCH", auth: true, body: JSON.stringify(payload) });
      } else {
        saved = await apiFetch("/api/counters", { method: "POST", auth: true, body: JSON.stringify(payload) });
      }
      setOk("Saved.");
      onSaved(saved);
    } catch (e2) {
      setErr(e2?.message || "Failed to save counter.");
    } finally {
      setSaving(false);
    }
  };
  ...
  return (
    <Modal title={isEdit ? "Edit Counter" : "Create Counter"} onClose={onClose}>
      {err && <Banner type="err">{err}</Banner>}
