...
const PAGE_SIZE = 10;
const TYPES = ["RETURN", "PAIDUP"];
const METHODS = ["CASH", "CARD", "UPI", "BANK", "CHEQUE", "TRANSFER", "OTHER"];

export default function ReturnPaidup() {
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      try {
        ...
        // Backend should support these query params. Example: GET /api/return-paidup?from=&to=&q=&page=&limit=
        const res = await apiFetch(`/api/return-paidup?${params.toString()}`, { auth: true });
        ...
        if (!ignore) { setErr(e?.message || "Failed to load Return / Paid-up entries."); setRows([]); setTotal(0); }
      } finally {
  ...
  const filtered = useMemo(() => {
    ...
    return rows.filter(r =>
      [r.type, r.bookingCode, r.guestName, r.mobile, r.method, r.reference, r.notes]
        .filter(Boolean)
  ...
  const kpis = useMemo(() => {
    ...
    const byType = TYPES.reduce((m, t) => ({ ...m, [t]: 0 }), {});
    const sum = list.reduce((n, x) => {
      const a = Number(x.amount) || 0;
      if (x.type && byType[x.type] !== undefined) byType[x.type] += a;
      return n + a;
    }, 0);
    return { count: list.length, totalAmt: sum, byType };
  }, [dataToRender]);
    ...
    <div className="page" style={{ display: "grid", gridTemplateColumns: "auto 1fr" }}>
      ...
      <div className="res-wrap">
        ...
        <div className="res-topbar">
          <h2 style={{ margin: 0 }}>Return / Paid-up</h2>
          <div style={{ display: "flex", gap: 8, flexWrap: "wrap" }}>
            <input
              className="res-select"
              placeholder="Search (type / guest / booking / method / ref / notes)"
              value={q}
            ...
            <button className="btn" onClick={openCreate}>+ Add Entry</button>
          </div>
        </div>
        ...
        <div className="kpi-grid" style={{ gridTemplateColumns: "repeat(auto-fit, minmax(180px, 1fr))" }}>
          ...
          <KPI label="Return Total" value={fmtMoney(kpis.byType.RETURN)} icon="↩️" />
          <KPI label="Paid-up Total" value={fmtMoney(kpis.byType.PAIDUP)} icon="✅" />
        </div>

        {/* Table */}
        <div className="panel" style={{ marginTop: 12 }}>
          <div className="panel-h">
            <span>Return / Paid-up List</span>
              ...
              <table className="table">
                ...
                <tbody>
                  {(!dataToRender || dataToRender.length === 0) && !loading && (
                    <tr className="no-rows"><td colSpan={11}>No entries found</td></tr>
                  )}
      ...
      {showDelete && (
        <ConfirmModal
          title="Delete Entry?"
          message={`Delete ${toDelete?.type || "entry"} for "${toDelete?.guestName || "Guest"}" (₹${Number(toDelete?.amount || 0).toFixed(2)})? This cannot be undone.`}
          confirmText="Delete"
          onClose={() => { setShowDelete(false); setToDelete(null); }}
          onConfirm={async () => {
            const id = toDelete?._id || toDelete?.id;
            await apiFetch(`/api/return-paidup/${id}`, { method: "DELETE", auth: true });
            afterDelete(id);
          }}
        />
      )}
...
/* ---------- Form Modal ---------- */
function ReturnPaidupForm({ initial, onClose, onSaved }) {
  ...
  const [type, setType] = useState(initial?.type || "RETURN");
  ...
  const onSubmit = async (e) => {
    ...
    try {
      ...
      if (isEdit) {
        ...
        saved = await apiFetch(`/api/return-paidup/${id}`, {
          method: "PATCH", auth: true, body: JSON.stringify(payload),
        });
      } else {
        saved = await apiFetch("/api/return-paidup", {
  ...
  return (
    <Modal title={isEdit ? "Edit Return / Paid-up" : "Add Return / Paid-up"} onClose={onClose}>
      {err && <Banner type="err">{err}</Banner>}
      ...
      <form onSubmit={onSubmit} style={{ display: "grid", gap: 12 }}>
        ...
        <Row>
          <Field label="Property Code (optional)">
            <input className="input" placeholder="If you scope by property" value={propertyCode} onChange={e => setPropertyCode(e.target.value)} />
          </Field>
          <Field label="Notes">
...
function Tag({ value }) {
  ...
  return (
    <span style={{
      ...
      borderRadius: 999, background: on ? "#eef2ff" : "#fef3c7",
      border: `1px solid ${on ? "#c7d2fe" : "#fde68a"}`,
      color: on ? "#3730a3" : "#92400e", fontSize: ".75rem", fontWeight: 700
    }}>
      {v || "—"}
    </span>
  );
